...
test("evolution capability-spec uses the typed interaction-backed review route", () => {
  const state = createInitialState();
  const sent = [];
  ...
  handleSubmitText(
    state,
    `/evolution capability-spec ${candidateId}`,
...
test("evolution capability-govern uses the typed human-governance route", () => {
  const state = createInitialState();
  const sent = [];
  ...
  handleSubmitText(
    state,
    `/evolution capability-govern ${candidateId}`,
    (type, payload) => sent.push({ type, payload }),
  );
...
test("reflection commands use the shared slash channel instead of typed review rejection", () => {
  ...
  const promotionPackageId = `evpromopkg_${"d".repeat(24)}`;
  ...
  handleSubmitText(
    state,
    `/evolution approval-requirement ${promotionPackageId}`,
    send,
  );
  handleSubmitText(
    state,
    `/evolution approval-request ${approvalRequirementId} security_reviewer`,
    send,
  );
  handleSubmitText(
    state,
    `/evolution approval-principal show ${approvalPrincipalId}`,
    send,
  );
  handleSubmitText(
    state,
  ...
  assert.deepEqual(sent, [
    ...
    },
    {
      type: "submit",
      payload: {
        text: `/evolution approval-requirement ${promotionPackageId}`,
      },
    },
    {
      type: "submit",
      payload: {
        text: `/evolution approval-request ${approvalRequirementId} security_reviewer`,
      },
    },
    {
      type: "submit",
      payload: {
        text: `/evolution approval-principal show ${approvalPrincipalId}`,
      },
    },
    {
      type: "submit",
...
test("Harness Live Eval keeps paid Tool execution and opens typed progress", () => {
  ...
  reduceServerEvent(state, {
    ...
    payload: {
      ...
      provider_response_ids_observed: 1,
      duration_ms: 10,
...
test("Harness Sandbox retry control commands stay on the shared Slash channel", () => {
  for (const command of [
    ...
    "/harness eval sandbox retry-retention-preview --retention-days 30 --limit 5 --scan-limit 10",
...
test("Pursuit terminal retention preview stays on the shared Slash channel", () => {
  const state = createInitialState();
  const sent = [];
  const command = "/pursue outbox retention-preview --retention-days 45 "
    + "--limit 5 --scan-limit 10 --assessed-at 2026-08-11T08:00:00+08:00";
...
test("Pursuit terminal retention admission stays on the shared Slash channel", () => {
  const state = createInitialState();
  const sent = [];
  const digest = "a".repeat(64);
  const command = `/pursue outbox retention-admit ptorpv_${digest.slice(0, 24)} `
    + `${digest} --retention-days 45 --limit 5 --scan-limit 10 `
