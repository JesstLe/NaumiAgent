...
export function handleSubmitText(state, text, send) {
  ...
  if (
    /^\/evolution\s+(?:experiment-contract|evaluation-contract|evaluation-final|decision-input|mechanical-gate|independent-review|counterfactual|reward-hacking|decision-state|decision-resolve|reflection|reflection-revoke|promotion-input|promotion-package)(?:\s|$)/i.test(
      commandText,
    )
  ) {
    return submitUserMessage(state, commandText, send);
