...
test('tool output survives run completion and reload with multiline detail', async ({ page }) => {
  ...
  await expect(trace.getByRole('button', { name: /任务已完成 · 用时/ })).toBeVisible()
  await trace.getByRole('button', { name: 'read 已完成' }).click()
  await expect(trace.locator('.bui-tool-output .rich-code pre')).toContainText(output)
  await expect(trace.locator('.bui-tool-output .rich-code pre')).toHaveCSS('white-space', 'pre')
  await page.reload()
  await expect(trace.getByRole('button', { name: /任务已完成 · 用时/ })).toBeVisible()
  await trace.getByRole('button', { name: 'read 已完成' }).click()
  await expect(trace.locator('.bui-tool-output .rich-code pre')).toContainText(output)
