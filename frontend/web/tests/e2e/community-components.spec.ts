...
test('workspace tree and AI sources use real response data', async ({ page }) => {
  ...
  await page.route('**/sessions/*/messages?*', route => route.fulfill({ json: {
    total: 1,
    messages: [{ id: 'answer', role: 'assistant', content: '引用已核对。', timestamp: '2026-09-11T10:00:00+08:00', metadata: { citations: [{ id: 'docs', title: '官方文档', url: 'https://example.com/docs', snippet: '这是服务端返回的真实来源摘要。' }] } }],
  } }))

  await page.goto('/web2')
  await expect(page.locator('.w2-message.assistant')).toContainText('引用已核对。')
