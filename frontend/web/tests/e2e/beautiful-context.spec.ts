...
test('context cards retain official foundation and refresh errors recover', async ({ page }) => {
  ...
  await summary.getByRole('button', { name: '上下文', exact: true }).click()
  const card = summary.locator('.rounded-card')
  ...
  fail = true
  await summary.getByRole('button', { name: '刷新上下文快照' }).click()
  await expect(summary.getByRole('alert')).toContainText('后端服务错误')
...
test('dependency dragging moves its connector and insights inspect actual status groups', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '切换对话摘要' }).click()
  const summary = page.getByRole('region', { name: '对话摘要' })
  await summary.getByRole('button', { name: '依赖', exact: true }).click()
  const graph = summary.getByLabel('任务依赖图')
  ...
  await summary.getByRole('button', { name: '洞察', exact: true }).click()
  const insights = summary.getByLabel('会话洞察')
...
test('empty summaries do not display upstream sample data', async ({ page }) => {
  ...
  const summary = page.getByRole('region', { name: '对话摘要' })
  await summary.getByRole('button', { name: '上下文', exact: true }).click()
  await expect(summary).toContainText('当前会话暂无上下文快照')
  await summary.getByRole('button', { name: '依赖', exact: true }).click()
  await expect(summary).toContainText('当前会话暂无待办依赖')
  await summary.getByRole('button', { name: '洞察', exact: true }).click()
  await expect(summary).toContainText('当前会话暂无可统计记录')
  await expect(summary).not.toContainText('Vanilla')
})
