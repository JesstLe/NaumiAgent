...
export type SelectionActionsProps = {
  ...
  externalAnchor?: { x: number; y: number };
};
...
export default function SelectionActions({
  ...
  const place = useCallback(() => {
    if (externalAnchor) { setAnchor(externalAnchor); setPositioned(true); return; }
  ...
  const run = (nextAction: string) => {
    if (onAction) { onAction(nextAction); return; }
  ...
  return (
    <div className="w-full max-w-[460px]">
      <div ref={hostRef} className="relative select-none pb-12">
        <p className="text-[13px] leading-relaxed text-ink" style={externalAnchor ? { display: 'none' } : undefined}>
          {passage.lead}
          <span
            ref={selectionRef}
        ...
        >
          ...
          <div
            ...
            style={{
              maxWidth: externalAnchor ? 'calc(100vw - 16px)' : undefined,
              overflowX: externalAnchor ? 'auto' : undefined,
              scrollbarWidth: externalAnchor ? 'none' : undefined,
              width:
                ...
                >
                  <form
                    ...
                    onSubmit={(event) => {
                      ...
                      run(prompt.trim() || actions.primary[0]?.action || "Improve");
                    }}
                  >
                    <input
                      value={prompt}
