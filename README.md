...
| Harness | `/harness status` `/harness eval … --repeat 5` `/harness baseline <suite>` `/harness baseline promote …` `/harness baseline compare …` `/harness explain` `/harness replay` | 实时显示 Candidate 评测/保存进度，在 typed 状态页查看并晋升/比较权威 Baseline，解释并安全回放运行 |
