...
**状态：已完成并推送。** CheckRunner、Git tree fingerprint、success cache、
...
**状态：已完成并推送。** 严格 Contract、task kind 机械升级、scope/Todo/check/
evidence/failure Gate、一次纠正与 verified/unverified/blocked Receipt 已通过定向测试。
...
**状态：已完成，等待本提交推送。** 同步/流式 final 均在 Todo 对账后进入 Gate；提前
完成文本会被隐藏，当前 run/profile/tree 检查通过后才发出类型化 Harness 回执。
...
**状态：已完成，等待本提交推送。** detached 临时 worktree 使用真实
`src/naumi_agent/harness/completion.py` 和真实 `pytest` 小模块完成验收：首次跳过检查被
Gate 纠正，检查通过后得到 `completed_verified`；检查后修改立即变为 stale；Profile
变化后命令未启动。集成场景 1 项通过，且临时 worktree 已清理。
