...
- 非目标：删除 Typer 管理命令、重写 Textual、富文本渲染、加载动画、全量文档治理
...
删除 `--classic` 路由，新增 `naumi tui`，统一默认入口，启动失败自动回退。保留尚被共享代码与
测试依赖的 Prompt Toolkit 实现文件，但它们不再是用户可达产品入口。该方案改动边界清晰，
...
- `_chat()` 及纯 Prompt Toolkit 文件的物理删除，如果静态依赖图仍显示引用。

后续“目录与文档治理”切片将先把共享模块迁移到中性命名空间，再删除确认不可达的 Prompt
Toolkit 前端、测试和依赖。当前切片的成功标准是旧交互入口不可达，不是伪装成一次目录清理。
