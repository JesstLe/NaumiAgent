...
- 助手消息的“重新生成”现在复用原用户消息和原回答位置。执行开始时，该消息上方的执行组件立即切换到本次实时过程；完成后绑定最新运行记录，重新计算用时与完成状态。
- 新回答替换旧回答，不在当前页面重复插入相同用户消息；回答时间随本次生成更新。重新生成不会清空输入框中尚未发送的草稿，也不会消费当前选择的附件。
- 历史运行匹配改为优先采用与该用户请求相符的最新记录，较早消息仍保留各自最近一次有效运行状态。
