...
- 当前 Validation Plan v1 未保留 Mutation Receipt 的 `modify/create` operation；因此本 lane 对 baseline 中
  不存在的 create path 保持 `baseline_path_missing`，不会擅自把缺失文件解释为 count=0。后续应先增加
  防篡改 file-operation binding，再覆盖新建文件的 RED fixture；
