...
- 计划尚未把 check kind 绑定到可信 Harness Profile 的具体 check ID/argv digest；
- 尚未建立 baseline/candidate 两个 HAR-08 cohort，也未产生 Comparison Receipt；
- Sandbox/真实命令执行仍等待 ARC-04 authority，不能由本模块自行放宽；
- 下一最小切片应实现 Profile check binding：要求类别必须被唯一、可信、与 changed path 匹配的 check
  覆盖，缺失或歧义时计划保持不可执行；随后再接 HAR-08 baseline/candidate runner。
