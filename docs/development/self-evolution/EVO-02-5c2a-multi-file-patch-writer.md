...
进程终止、断电或 `BaseException` 形成的遗留事务由下一切片 EVO-02.5c2b 启动恢复；本切片负责确保崩溃
前已经留下足够、准确、可扫描的 intent。
...
- 尚未有 `EvolutionPatchSetRecoveryCoordinator`，启动时只会恢复旧的单文件 journal；
- 多文件恢复结果尚未合并到 TUI/新 UI patch recovery payload；
- postflight 当前验证完整文件摘要和精确 Git scope，尚未做 EVO-02.6b 的 API surface/完整 diff 策略；
- SQLite 与文件系统依靠 intent + digest 恢复，不宣称跨介质 ACID；
- Windows 占用文件、磁盘满、断电和目录同步语义仍需平台矩阵。

下一切片 EVO-02.5c2b 必须复用相同锁和 write-set store，覆盖 prepared/applying/applied/rolling_back 的
真实磁盘状态组合，并把脱敏恢复结果接入 Engine、TUI 与新 terminal UI。
