# EVO-05.7b3b2b Authenticated Observation Revision HTTP Transport

## 目标与边界

[EVO-05.7b3b2a](EVO-05-7b3b2a-fenced-observation-revision-delivery-worker.md) 已完成 durable dispatch、owner/epoch/lease
...
本切片只把 7b3b1 signed Submission 传到 Control Plane，并返回 exact 7b3b1 Receipt。它不读取长期 heartbeat window、不计算 fleet
...
1. Submission 已由 current Population Credential 对独立 domain
   `naumi.release.stable-promotion-observation-revision.v1` 签名；
...
```text
POST /v1/stable-promotion/observation-revisions:receive HTTP/1.1
Content-Type: application/vnd.naumi.stable-promotion-observation-revision.v1+base64
Accept: application/vnd.naumi.stable-promotion-observation-revision-receipt.v1+base64
X-Naumi-Admission-ID: evstablepromadmit_...
X-Naumi-Submission-ID: evstablepromrevsubmit_...
Content-Length: ...
Connection: close
...
`extra=forbid` 和 2 MiB artifact 上限；signed Submission payload 仍受 64 KiB 限制，HTTP envelope 进一步限制 128 KiB
request/response。
...
- Admission/Revision Submission header 与 body artifact identity 不一致；
...
归为 retryable，协议、identity、artifact 和 authority `4xx` 归为 permanent，由 7b3b2a Worker 按 durable budget settlement。
...
- endpoint semaphore 限制并发 handler，容量耗尽时不会进入 7b3b1 receive Service；
...
installation 出站配置位于 `harness.stable_promotion_observation_revision_http_transport`：
...
harness:
  ...
  stable_promotion_observation_revision_http_transport:
    enabled: true
    endpoint_url: https://control.example/v1/stable-promotion/observation-revisions:receive
...
Control Plane server 使用 `StablePromotionObservationRevisionHTTPServerPolicy` 显式注入 server cert/key、installation CA、
...
- [x] signed Revision 经 7b3b2a Worker 跨 TLS 到达 7b3b1 Control Plane Service 并形成 Receipt ACK；
...
- [x] Runtime config 自动装配 7b3b2a Worker transport，partial/unsafe timeout 配置失败关闭；
- [x] public lazy API、Ruff、compile、YAML、diff check 与定向 pytest 通过；
- [x] 按用户要求未运行全量测试。

## 自我审视与下一步

本切片使 Observation Revision 真实跨越认证网络边界，但没有扩大观测结论：
