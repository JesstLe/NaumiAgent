...
- 本切片不是完整 RED cohort；后续 orchestrator 必须按连续 sample index 逐项调度并生成 cohort completion
  receipt。
- metric runner 仍未在同一精确 revision 环境中执行；下一小切片应先为 ready metric 设计可与 Profile
  check sample 合并的 typed observation adapter，再扩展到完整 RED cohort。
