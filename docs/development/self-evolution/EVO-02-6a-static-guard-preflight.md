...
- EVO-02.5a 已实现受 Guard Receipt 约束的单文件原子 Writer；多文件 journal、完整 postflight 与
  EVO-02.7 Mutation Receipt 尚未实现。

Writer 会在持锁后重跑本 preflight 并要求 Receipt 完全一致；写后再核对文件摘要与 Git scope，失败则在
隔离 worktree 内回滚。详见 `EVO-02-5a-single-file-patch-writer.md`。
