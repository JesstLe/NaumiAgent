...
- 尚未实现 EVO-02.5 Patch Writer、EVO-02.7 Mutation Receipt。

下一切片可以实现 EVO-02.5a：只接受 `preflight_passed=true` 且 proposed content digest 与 Guard Receipt
完全相同的原子 Patch Writer；写后必须立即重跑 Static Guard postflight，失败则在隔离 worktree 内回滚。
