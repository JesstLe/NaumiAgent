...
EVO-03.3c2 使用同一 Snapshot 构造 candidate Sandbox Check：把 blobs 物化到 ARC-04 临时 revision，执行与
RED 相同的 Profile check 和 metric runner，并在首次 H5a 写入前再次复验 Snapshot。完整 cohort 编排仍留给
后续独立切片。
