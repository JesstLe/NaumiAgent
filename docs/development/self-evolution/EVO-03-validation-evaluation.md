...
UI-12.3b3/3b4 已交付最长 3600 秒、绑定 Harness Run Lease fence 的可撤销运行委托 authority，并接入
短期 delegated receipt 与 ExecutionGrant 的签发/dispatch 复验；Shell admission 与 cohort executor 尚未
接入。Interventional Harness runner、adversarial suite 与最终 Evaluation Receipt 仍未实现，因此 EVO-03
整体保持 partial。
