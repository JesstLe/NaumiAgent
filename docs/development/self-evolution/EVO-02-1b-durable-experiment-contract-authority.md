...
- EVO-04.2a Mechanical Gate 与 4.3a Independent Reviewer 已实现；counterfactual、reward-hacking
  detector、decision state 与 promotion 尚未实现。

## 下一步

EVO-04.2a 与 4.3a 已依次消费 Decision Input 和 Gate authority。下一步进入 EVO-04.4a Counterfactual
Evidence Contract：机械寻找更小 scope、删测试、放宽指标、skip/mock 与评测泄漏等替代解释；仍不做最终
accept/reject。
