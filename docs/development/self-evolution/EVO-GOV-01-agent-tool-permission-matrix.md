...
EVO-03.7a/3.7b1/3.7b2 与 EVO-04.1a/4.2a/4.3a/4.4a 已注册七个非只读 Agent Tool。它们拥有真实的 durable
...
七个 Tool 都只能从已有签名 authority 派生并持久化不可变证据：
...
- lockdown：阻断所有七类派生写入；已有只读 Authority Tool 仍按各自只读规则工作。
...
- 七个 Tool 在 permissive/moderate/strict 均返回 `ALLOW + MEDIUM`，family 精确匹配；
