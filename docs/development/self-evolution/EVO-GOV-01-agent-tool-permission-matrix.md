...
EVO-03.7a/3.7b1/3.7b2、EVO-04.1a 至 4.7a 与 EVO-05.1a/1b/2a/2b 已注册十六个非只读 Agent Tool。它们拥有真实的 durable
...
Promotion Input、review-only Package、不可执行 Approval Requirement 或 role response：
...
因此十五类派生创建为 `MEDIUM`：高于只读查询，但不逐次要求确认。Reflection 撤销是独立 `HIGH` 治理动作：
...
- permissive/moderate/strict：十五类派生创建允许且无逐次确认；Reflection 撤销允许但要求确认。
- lockdown：阻断所有十六类写入；已有只读 Authority Tool 仍按各自只读规则工作。
