...
EVO-03.7a/3.7b1/3.7b2、EVO-04.1a 至 4.7a 与 EVO-05.1a 已注册十三个非只读 Agent Tool。它们拥有真实的 durable
...
其中十二个 Tool 只能从已有签名 authority 派生并持久化不可变证据、决策、用户 Resolution、Reflection 或
Promotion Input：
...
因此十二类派生创建为 `MEDIUM`：高于只读查询，但不逐次要求确认。Reflection 撤销是独立 `HIGH` 治理动作：
...
- permissive/moderate/strict：十二类派生创建允许且无逐次确认；Reflection 撤销允许但要求确认。
- lockdown：阻断所有十三类写入；已有只读 Authority Tool 仍按各自只读规则工作。
...
- 十二个派生 Tool 在 permissive/moderate/strict 返回 `ALLOW + MEDIUM`，family 精确匹配；
