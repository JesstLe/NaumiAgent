...
- 完整 Diff/API Postflight、EVO-02.7a Mutation Receipt Core、EVO-02.7b1 Generation Trace 与 2.7b2
  Trace→Receipt v2 绑定已在后续切片实现；EVO-02.7c1 也已完成专用 Mutation Turn Runner，尚缺
  HAR-08 RED/GREEN 对 Mutation Receipt v2 的真实消费。
...
链路复用同一持久状态机和锁，没有循环调用单文件 Recovery 冒充多文件恢复事务。
