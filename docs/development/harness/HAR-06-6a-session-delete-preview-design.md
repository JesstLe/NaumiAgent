...
预览明确说明：本切片不执行删除。当前 `/delete` 的跨 Store saga、legal hold、tombstone、
artifact GC 尚未完成；用户应把预览用于风险判断，而不是把它视为清理已安全落地。
...
HAR-06.1 policy、HAR-06.2 reconciliation、HAR-06.3 tombstone 和 HAR-06.4 artifact GC 完成后，
删除执行才能消费本预览模型，并在 commit 前后校验计数是否发生漂移。
