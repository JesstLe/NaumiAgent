...
Artifact 数量依旧只是引用快照，不等于可删除文件数。HAR-06.4 会在执行时重新归一化、去重、
复核存活引用和路径类型，只删除受管根目录中的无共享普通文件。
...
HAR-06.4 artifact GC 已消费协调记录中的不可变引用快照，并重新校验 workspace、Session、共享引用
和文件类型。HAR-06.5 再把恢复与 retention 扩展为有界周期 worker；两者都不改变本预览的只读语义。
