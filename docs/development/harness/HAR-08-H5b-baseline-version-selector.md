...
本切片本身不保存 before/after Comparison receipt，不开放 Slash/API/UI，也不自动选择历史最好结果；
后续 H5c 已在同一 HarnessStore 中补齐不可变 Comparison receipt。
...
- H5c 已完成：不可变 Comparison receipt 引用 baseline/current batch、逐样本证据与全部 verdict digest；
