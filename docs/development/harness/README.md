...
HAR-10.8f2k 已交付默认 dry-run、bypass 显式物理 prune、逐写点事务回滚、完成回执与永久防复活 tombstone。
HAR-10.8f2l 又交付 snapshot-bound disposed cursor、四端分页与新旧客户端能力协商降级。
独立加密 archive/外部 anchor 与 push stream 仍未完成。
...
4:2:1:1 公平轮转、旧账本哈希兼容，以及 New UI/TUI/Goal 同源投影；HAR-10.6d 已进一步补齐
snapshot-bound pending recovery cursor、跨页公平轮转、New UI 50 卡有界补位和 TUI 后续页恢复。
...
authority 均保持关闭。HAR-09.6e2 已继续将 current head 与 immutable root 分离为 Proposal Outcome Projection v2，
并同源同步 Workbench/New UI/Textual TUI；长期窗口、supersede event 和动态撤权现可直接审查。promoted Outcome 与
配置/数据 rollback 仍未完成。
