...
- schema/runner 单元测试覆盖成功、实现 mismatch、asset error、extra/unknown runner、危险路径、
  symlink escape、invalid JSON、超限、重复 ID、digest mismatch 与 suite budget。
- `/harness eval` 经共享 slash router 在 Engine 表面运行；`harness_eval` Tool 调用相同 Service，
  连续两次 canonical payload 一致。
- 当前仓库 Profile 声明 `protocol-hello-core`，六个 fixture 直接调用生产
  `normalize_client_record()` / `negotiate_hello()`，全部通过。
- 真实工作区运行前后 Profile、Suite、fixture 与生产协议源码 digest 完全一致；没有模型、API key、
  命令、网络或工作区写入。
- Python 定向测试、Node Harness 命令元数据测试、Ruff、compileall 与 282 份文档治理检查通过。

## 自我审视与未完成项

- 本实现有真实 schema、文件 integrity、生产代码执行和双通道表面，不是 Prompt 包装。
- Eval error 与 implementation failure 已机械分离；结果不保存 fixture 正文、用户对话、secret 或
  reasoning。
- 当前 runner 只覆盖 ARC-03 hello 协商。通用 JSON Schema、Harness Evidence、Replay、Sandbox、
  Live 和 LLM Judge runner 均未实现，不能把六个 case 外推为整体质量分数。
- canonical result 主动排除 duration，确保同输入可比较；性能预算仍保留在单次结果中，未来
  Comparator 必须用多次样本统计，不能比较一次 wall-clock。
- Suite/expected 仍是仓库可修改资产，SHA-256 只防止 fixture 与声明漂移，不提供 baseline promotion
  审批；HAR-08.6/8.7 完成前不得用于自动提升自进化补丁。
- 当前真实场景在 macOS 验证；runner 本身没有 shell/path 分支，但 Linux/Windows CI 证据仍缺失，
  因而 HAR-08 的 A4 三平台标准尚未满足。
- API、结果 Store、baseline identity 和 comparator 仍未实现；HAR-08 保持 partial。
