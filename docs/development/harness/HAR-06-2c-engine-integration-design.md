...
本切片把 HAR-06.2b/6.3b 的协调器接入实际运行路径，不扩展 Artifact 文件删除：
...
5. 本轮不启动常驻定时 worker，不删除 Artifact 文件，也不改变删除预览的只读语义。
...
|---|---|---|
| `completed` | 已删除并完成派生记录协调 | `204 No Content` |
...
- HAR-06.4：Artifact 引用归一化、共享引用检查、路径边界验证与物理文件 GC。
- HAR-06.5：周期 retention worker、批次预算、取消、空间上限与观测指标。
- 因此“Session 与 Harness 数据库行已完成协调”可以成立；“所有 Artifact 已清理”仍不成立。
