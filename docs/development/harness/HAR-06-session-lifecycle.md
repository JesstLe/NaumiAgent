...
- HAR-06.5 Retention worker：按天数、空间上限和最近访问执行有界清理。
  ...
  - **HAR-06.5b2 待实现**：周期调度、单实例租约、关闭 drain、抖动/退避与吞吐观测。
...
HAR-06 当前为 `partial (6.1, 6.2a-6.5b1, 6.6a)`。已落地的前置能力包括：
...
HAR-06.5a/6.5b1 已实现 10k 级候选扫描、空间/时间资格、只读计划和显式有界执行，但尚未启动后台清理。当前 `/delete`
已协调 Session、Harness 数据库行和安全可删除的受管 Artifact；共享、风险、非普通文件及受管根目录
外引用会保留，并在协调记录中计数。周期调度、逐批提交、主动取消、时间预算和观测指标仍由
HAR-06.5b2 完成，详见 `HAR-06-4-artifact-gc-design.md`、`HAR-06-5a-retention-preview-design.md`
与 `HAR-06-5b1-retention-pass-design.md`。
