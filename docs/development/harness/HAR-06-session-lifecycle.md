...
- HAR-06.5 Retention worker：按天数、空间上限和最近访问执行有界清理。
  ...
  - **HAR-06.5b2b 已实现**：New UI Bridge、TUI、旧 CLI 与 API 在恢复成功后按配置启动，
    一次性任务明确禁用；runtime status/API health 共用类型化状态，所有入口关闭时 drain。
...
HAR-06 当前为 `implemented (6.1-6.6a)`。已落地能力包括：
...
- 默认 New UI Bridge、TUI fallback、deprecated 旧 CLI 与 API lifespan 均先完成 Session 协调恢复，
  再按 `periodic_enabled` 启动 worker；恢复失败不启动且释放 Engine。一次性任务不启动周期服务。
  New UI 在 ready 与每次心跳后的 `runtime/status` 获取快照，API health 返回同一类型化状态。
- `/delete`、历史面板删除、Agent Tool 与 API DELETE 现均经由协调器执行；API 对完整成功、
  安全重试与重试耗尽分别返回 204、202 与 503，终端界面显示对应中文状态。
- CLI、任务模式、New UI Bridge、TUI 与 API 启动时执行一次有界恢复扫描。Session Store 是运行时权限的权威：
...
HAR-06.5a/6.5b1/6.5b2 已实现 10k 级候选扫描、空间/时间资格、只读计划、显式有界执行和
默认关闭、明确启用后自动挂载的租约周期服务。`/delete` 已协调 Session、Harness 数据库行和安全
可删除的受管 Artifact；共享、风险、非普通文件及受管根目录外引用会保留并持久计数。详见
`HAR-06-4-artifact-gc-design.md`、`HAR-06-5a-retention-preview-design.md`、
`HAR-06-5b1-retention-pass-design.md`、`HAR-06-5b2a-periodic-core-design.md` 与
`HAR-06-5b2b-long-running-lifecycle-design.md`。
