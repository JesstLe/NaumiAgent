...
- HAR-06.2 Reconciliation hook：Session 操作成功后触发幂等 Harness 清理。
  ...
  - **HAR-06.2b 已实现**：独立于 Engine 的协调器按持久状态执行 Session/Harness 两阶段删除，
    并以确定性 request id 支持跨进程恢复；Engine hook 接入仍待 6.2c。
- HAR-06.3 Tombstone：删除失败时记录安全 tombstone，后台重试。
  ...
  - **HAR-06.3b 已实现**：恢复协调器发现无 tombstone 的 crash gap、领取到期任务、从精确状态
    续跑并安全解决或再次记录失败；启动/定时生命周期接入仍待 6.2c/6.5。
...
HAR-06 当前为 `partial (6.1, 6.2a-6.2b, 6.3a-6.3b, 6.6a)`。已落地的前置能力包括：
...
尚未实现 HAR-06.2c、HAR-06.4-06.5；因此当前 `/delete` 仍只处理 Session Store，不会把本预览中的 Harness
