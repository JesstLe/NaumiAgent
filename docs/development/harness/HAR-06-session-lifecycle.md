...
HAR-06 当前为 `partial (6.1, 6.2a-6.2c, 6.3a-6.3b, 6.6a)`。已落地的前置能力包括：
...
尚未实现 HAR-06.4-06.5。当前 `/delete` 已协调删除 Session 与精确 workspace/session 下的
Harness 数据库行，但只保存 Artifact 引用快照，不删除外部文件。引用安全 GC 与周期性 retention
worker 完成前，不得宣称物理 Artifact 已清理，详见 `HAR-06-2c-engine-integration-design.md`。
