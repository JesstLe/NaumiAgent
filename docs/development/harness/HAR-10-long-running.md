...
- HAR-10.7 Cluster scheduling（partial）：
  ...
  - HAR-10.7h1 / ARC-04.5e1 已建立真实 spawned Agent 控制进程、一次性 authkey/nonce 本机认证、
    ...
    manifest、加密 Tool RPC、多轮模型循环和重复副作用阻断，但生产调度仍未完成。见
    ...
    [Tool RPC](../architecture/ARC-04-5e3b1-encrypted-agent-tool-rpc.md)。
  ...
  - ARC-06.2b2 已让 claimed ToolJob 进入真实 Shell dispatch/start/terminal 链，并在 start 前复验
    ...
    RPC 与多轮执行内核。生产 scheduler/authority adapter 和完整 Supervisor 尚未完成，因此仍只是
    HAR-10.7 集群调度前置。
