...
- HAR-10.3 Durable queue（partial）：
  ...
  - 未完成：Bridge 接线、claim lease/fencing、重启恢复、跨客户端公平性、优先级、cursor、retention 与取消传播。
...
当前事务的 New UI 队列提升，HAR-10.3b1 已交付尚未接线的 durable queue Store 核心；同步
shell、browser/agent/runtime 逐域接入、Goal interaction actions、跨进程/跨 Store 原子性、heartbeat 多域
接入和 durable queue runtime authority 仍属于后续实现。
