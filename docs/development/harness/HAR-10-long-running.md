...
- HAR-10.3 Durable queue（partial）：
  ...
  - 未完成：历史 claim 人工 resolve、TUI 运行中输入队列、跨客户端公平性、优先级、cursor、retention 与取消传播。
...
当前事务的 New UI 队列提升，HAR-10.3b1/3b2 已交付 durable queue Store 与 New UI Runtime 接入；同步
shell、browser/agent/runtime 逐域接入、Goal interaction actions、跨进程/跨 Store 原子性、heartbeat 多域
接入、queue 人工 resolve 与 TUI queue parity 仍属于后续实现。
