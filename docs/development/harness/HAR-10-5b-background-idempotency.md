...
- HAR-10.4b 尚未根据 action ledger + BackgroundTask 状态自动解除 `reconcile_required`；
- browser、subagent、外部 API 仍没有各自的 idempotent job contract。

下一切片 HAR-10.5c 应实现类型化 background reconcile decision：机械区分 waiting、terminal、stale preparing、
dispatched-without-task 和 legacy unknown，只在证据充分时推进 checkpoint；未知状态继续 fail closed。
