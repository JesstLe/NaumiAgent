...
- 已消耗 token、成本、时间、三类预算上限，以及 stagnation/verify/plan/replan 运行参数；
...
HAR-10.4b 新增的运行参数采用缺省字段兼容：读取 HAR-10.4a 已落盘的旧 payload 时不把缺省字段加入
canonical JSON，因此旧摘要与 checkpoint ID 保持不变；新 checkpoint 会显式保存完整参数。
...
HAR-10.4a 只交付权威输入和安全判定。HAR-10.4b 已负责从 snapshot 重建 GoalSpec、历史、预算和下一安全
行动，并在新 lease epoch 下继续 planner loop；in-flight 外部副作用仍交由 HAR-10.5 reconcile。
...
HAR-10.4a 本身不恢复执行，也未解决 destructive action 的 durable idempotency、外部任务 reconcile、跨
Store 事务或人工交互回填。HAR-10.4b 已接入受 lease 保护的继续执行；下一切片 HAR-10.5 对
browser/background/process 的真实外部状态做 reconcile。
