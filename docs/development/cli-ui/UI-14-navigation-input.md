...
- UI-14.2 QuickOpen：partial；UI-14.2a 已交付 New UI/TUI 命令 provider、fuzzy/中文元数据搜索、
  ...
  workspace 隔离且最多 100k 文件的索引，两端文件 provider 只填入安全 `/read` 模板。Agent 和页面 provider
  尚未实现。
  ...
  `UI-14-2e-workspace-file-quick-open.md`。
...
UI-14.1a 已提供 QuickOpen 的命令事实源并改善现有 slash completion；UI-14.2a-14.2e 已提供两端
命令/任务/会话/文件 QuickOpen、本次启动的隐私安全最近命令排序、实时权威任务/会话快照与可取消工作区文件索引，
但尚未实现 Agent/页面 provider、跨启动历史、
