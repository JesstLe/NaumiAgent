...
UI-17.2a-17.2d 已用共享 fixture 锁定 Bridge、TUI 与 Node reducer 的 runtime-health 八字段语义、权限脱敏、
bypass/session grant 四选择、模型主动询问和 canonical answer、submit/tool/receipt/cancel 基本运行生命周期，以及
Evaluation Lane RED/GREEN、资源证据与强制非最终边界；
...
17.2c terminal run lifecycle、17.2d Evaluation Lane Receipt golden 已完成；17.3a 已验证 New UI 对旧 Bridge 的
Evaluation Lane typed→Slash 降级。仍需流式/error/retry 等 golden、通用 capability registry 和 17.3b-17.6。
