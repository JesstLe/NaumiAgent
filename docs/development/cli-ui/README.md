...
UI-17.2a 已用共享 fixture 锁定 Bridge、TUI 与 Node reducer 的 runtime-health 八字段语义。
两端不再依赖各自的临时排队状态。
后续模块不得绕开这些路径重建新状态层。
...
UI-17 是统一发布门；17.1 manifest 已完成，17.2a runtime-health golden 已完成，仍需其余 17.2 与 17.3-17.6。
