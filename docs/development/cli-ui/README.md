...
UI-17.2a-17.2c 已用共享 fixture 锁定 Bridge、TUI 与 Node reducer 的 runtime-health 八字段语义、权限脱敏、
bypass/session grant 四选择、模型主动询问和 canonical answer，以及 submit/tool/receipt/cancel 基本运行生命周期；
...
文件/会话/任务/Agent provider、最近使用与 Vim/input mode 尚未实现。
两端不再依赖各自的临时排队状态。
后续模块不得绕开这些路径重建新状态层。
...
17.2c terminal run lifecycle golden 已完成，仍需流式/error/retry 等 golden 和 17.3-17.6。
