...
- waiting Approval 的 approve/reject 动作（后由 UI-10.6e 独立交付；Approval 不支持 defer）；
...
因此 UI-10 与 HAR-09 仍为 `partial`。UI-10.6b2 后 Proposal defer/merge 入口齐备；waiting Approval
动作随后由 UI-10.6e 完成。其余状态仍以各自模块册为准。
