...
- UI-17.2 Golden scenarios：partial (17.2a)；runtime-health 已使用同一 fixture 对 Bridge、TUI 与 Node reducer 断言
  语义字段，见 [设计与验收](UI-17-2a-runtime-health-golden.md)；permission/interaction/tool/receipt 等仍待实现。
...
SQLite 验证两端 heartbeat/retention/Doctor/terminal 语义。UI-17.1 已建立可机读 manifest，UI-17.2a 已锁定
runtime-health golden；其余 golden scenarios 与 UI-17.3 compatibility negotiation 尚未完成，不能凭单项对照通过发布门。
