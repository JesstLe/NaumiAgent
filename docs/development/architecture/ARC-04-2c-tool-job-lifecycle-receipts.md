...
- ARC-04.3a 已消费本合同，完成 authenticated local non-PTY transport 与最小 OS 隔离；HAR-08.4a 也已通过
  相同 ToolJob/lifecycle authority 跑通一个真实 Profile check。下一步不得另造任务身份，应补齐外层权限回执
  向内层 Shell Job 的委托合同，再切换生产 surface。
