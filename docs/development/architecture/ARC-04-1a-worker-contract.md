...
下一步 ARC-04.1b 应实现受信 worker registration store 与 incarnation/fencing authority，使合同摘要不只是调用方
传入值；随后 ARC-04.2 才能把 immutable job、permission grant、workspace lease 和 idempotency key 绑定到已注册
worker。ARC-04.3 的真实 shell daemon 完成并证明进程树、网络、环境和资源隔离之前，HAR-08.4 继续保持
planned，EVO-03.6 也不得运行项目命令。
