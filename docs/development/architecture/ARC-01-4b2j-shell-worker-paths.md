...
- `run_delegation_grant_db_path`：长周期 Harness/Evolution Run Grant 的独立、可撤销 SQLite authority，
  不与 immutable permission receipt 或 execution grant 混表。
...
- Engine 构造后，上述三个目录与 Run Grant 数据库仍不存在。
