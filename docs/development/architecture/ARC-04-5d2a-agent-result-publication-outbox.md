...
`delivery_sha256` 只证明 publisher 声明的精确投递事实。它不能单独证明下游已经持久消费；这一边界由
ARC-04.5d2b 的 sink/consumer receipt 定义。
...
所以本切片只建立可实现 at-least-once 的权威基础。ARC-04.5d2b 必须让每次投递携带稳定
`publication_id`，定义 sink 去重或明确允许重复，并且只有在目标 sink 达到约定 durable boundary 后
才能 acknowledge。未完成这些条件前禁止写“exactly-once”。
...
下一最小切片是 `ARC-04.5d2b Agent Result Publication Consumption`：

1. 先定义稳定 publication delivery envelope 和 sink 成功边界；
2. 让 production manager 通过 claim → recover → deliver → acknowledge 消费同一 authority；
3. 用崩溃点测试证明 commit-before-deliver、deliver-before-ack 与 restart takeover；
4. 明确 at-least-once 重复语义，不能用内存 flag 冒充持久幂等；
5. 完成后再选择 recovery UI，而不是让 UI 直接操作 outbox。
