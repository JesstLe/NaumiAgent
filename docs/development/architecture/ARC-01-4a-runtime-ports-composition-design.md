...
```text
session 必须实现完整的 SessionPort 契约：create_session/save/load/list_sessions/delete/archive/close
permission 必须实现完整的 PermissionPort 契约：mode/set_mode/check/reset_counts
model 必须实现完整的 ModelPort 契约：metadata/routing/capability/discovery/reasoning/call/stream
tool_execution 必须实现完整的 ToolExecutionPort 契约：invoke
events 必须实现完整的 EventSink 契约：emit
```
