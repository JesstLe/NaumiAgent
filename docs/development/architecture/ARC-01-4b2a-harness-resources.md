...
- ARC-01.4b2b-4b2e 已完成 Evolution、ChatRun、Task/Workbench 与 Goal/Pursuit Store；
  Background/Scheduler 仍未迁移；
...
因此 ARC-01.4 继续保持 partial；后续不能把 Browser/Runner 一次塞入 Resource bundle 而跳过生命周期
设计，也不应为完成编号机械迁移，须选择能解锁用户表面的最小前置。
