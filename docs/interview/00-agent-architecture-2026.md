...
全书的主要读者是准备阿里巴巴、腾讯等国内大厂 Agent 工程岗位的求职者。写作主线固定为“岗位要解决的问题 → NaumiAgent 的实际设计 → 个人可完成的改造 → 实验证据 → 面试表达”。不要在上下文交接后退回纯技术百科，也不要仅把公司名字贴到通用问答上。岗位事实和教材模拟场景的出处、适用范围见第 19 章。
...
每章还有动手练习、失败分支、验收标准与白板评分，不再重复设置“学完怎样写进简历”一节；完整的项目经历表达集中在项目介绍与求职训练章节。知识讲解至少达到约 1000–2000 中文字的实质深度，必要时更长；代码、链接和重复模板不应凑篇幅。章节增加体量时，应优先增加机制、反例和真实实验，而非同义句填充。

每章另设一组大厂项目深挖，必须覆盖四项内容：一个具体业务约束；一段区分已有实现与拟改进方案的回答；至少三个递进追问及回答方向；一项能形成个人贡献的实验及失败验收。每个问题都应能落到输入、状态、工具、权限或结果，不能只回答“加 Redis”“用多 Agent”“优化 Prompt”。提到生产规模、效果提升或个人主导时必须有相应证据。
