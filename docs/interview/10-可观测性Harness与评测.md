...
<!-- teaching-figure:10:start -->
![运行支撑与评测支撑分工：章节教学示意图](assets/illustrations/10-teaching-v2.png)

读图：左栏管理模型持续运行，右栏在固定条件下比较基线与候选。先产生工件，再由评分器判定，最后才有得分；独立评分器本身也需要检查覆盖范围和误判，不能把图中的勾理解成全项目已通过。
<!-- teaching-figure:10:end -->
