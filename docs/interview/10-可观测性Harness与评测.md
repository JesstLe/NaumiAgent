...
## 大厂项目深挖：你说改进有效，怎样证明不是挑了成功案例

场景模拟：候选人声称增加上下文摘要后 Agent 更稳定，面试者要求展示证据。应用、平台与研发效能岗位都需要回答这类验证问题；这里不假定任何公司的实际题频。

项目回答：“运行 Harness 帮助组织工具、状态和约束，评测 Harness 则在固定条件下检查任务表现，两者不能混为一谈。我会先冻结任务集和改动前基线，定义独立验收，再运行候选方案。NaumiAgent 的离线练习可以检查账本与文件不一致，但没有调用模型，因此不能证明模型成功率提升。真实模型评测还要记录配置、任务版本、重复运行和全部失败结果。”

连续追问一：“成功率到底怎么算？”先定义一个任务成功要同时满足哪些内容与安全条件，以纳入评测的任务尝试为明确分母，不能只统计成功返回的请求。对重试，既记录最终任务结果，也记录尝试次数和累计成本。取消、环境不可用和超时要预先约定分类，不能看到结果后临时删除困难样本。

连续追问二：“让另一个模型打分不就独立了吗？”换模型不自动消除偏差。文件是否存在、引用标识是否有效、是否发生未授权写入等应尽量由程序核验；开放内容质量可以用评审模型辅助，但需要评分规则、校准样本和人工抽查。被评 Agent 不应修改自己的验收规则。

连续追问三：“日志很全为什么还是定位不了？”日志数量不等于关联能力。至少要能串起任务、执行尝试、模型调用、工具动作和最终产物，并记录阶段耗时与错误类型。敏感参数要脱敏，不能为了排查把凭据复制到日志和模型上下文。

个人改造建议：针对一项真实补丁设计对照实验，保留正常、边界和故障三类任务，预先写清纳入规则。报告任务成功、安全失败、未验证、全部尝试成本和时延；小样本先展示逐例结果，不夸大百分比。再挑一个失败样本沿轨迹定位到具体函数，说明改的是模型策略、工具契约还是验收器。能解释一次失败为何算失败，比只展示漂亮总分更有说服力。
