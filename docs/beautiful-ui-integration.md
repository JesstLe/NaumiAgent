...
- Thinking 保留原版闪光、入场、折叠和详情过渡。发送消息时立即出现，按真实 turn/sequence 分成多个阶段；每阶段先组合现有 Steps、Reasoning、Search 或 Coding + Tool Chips，再显示透明的公开阶段摘要，最终回答位于所有阶段之后。摘要由后端 `phase_summary` 的真实工具动作和终态生成并持久化，不传输模型原始 reasoning；旧记录仅从已保存的 `public_action` 回退。工具详情支持多行和滚动，执行结束后从共同后端读取已保存预览，详见 [Web2 具体执行摘要](web2-public-activity.md)与[工具输出持久化修复](tool-output-persistence.md)。长输出仍受引擎事件预览长度限制。
