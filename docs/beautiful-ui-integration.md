...
- Thinking 保留原版闪光、入场、折叠和详情过渡。发送消息时立即出现，按真实 turn/sequence 将公开分析说明与工具调用交错展示，最终回答位于时间线之后；完成后保持展开并可手动收起。模型原始 reasoning 不进入 Web。工具区域不再次展示 Diff，详情支持多行和滚动。执行结束后从共同后端读取已保存预览，旧记录在关联证据明确时从会话恢复，详见 [工具输出持久化修复](tool-output-persistence.md)。长输出仍受引擎事件预览长度限制。
