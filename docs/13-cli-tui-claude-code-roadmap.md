...
- ✅ Doctor diagnostics screen：`ui.doctor` 提供 Python/config/API key/model/workspace/git/rg/browser daemon/docker/MCP/debug log/terminal 与 ARC-05.1 物理 Store Catalog 的确定性只读检查，CLI/TUI `/doctor` 与 Agent 工具 `doctor_diagnostics` 共用同一套诊断与 Markdown 报告；UI-13.3a 提供 New UI/TUI/CLI/Agent Tool 共用的显式受控 Provider 探测（最多 1 请求、8 输出 token、默认 15 秒超时、无自动重试、可取消）；UI-13.5a 进一步提供 typed 脱敏 ZIP，先预览固定文件集与 digest，再用精确 Snapshot 摘要原子写入平台状态目录。
...
ARC-04.5e3b2/HAR-10.7h6 又完成生产 SubAgent 默认独立路由、Engine permission/tool authority 和 Agent
Control schema v6 后端投影。下一步需比较长驻池/公平调度、流式事件与并行 Tool RPC 的用户收益；
跨终端兼容实测、性能基准和真实
终端截图回归仍需继续。
```
