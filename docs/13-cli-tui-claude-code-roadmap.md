...
- ✅ Doctor diagnostics screen：`ui.doctor` 提供 Python/config/API key/model/workspace/git/rg/browser daemon/docker/MCP/debug log/terminal 与 ARC-05.1 物理 Store Catalog 的确定性只读检查，CLI/TUI `/doctor` 与 Agent 工具 `doctor_diagnostics` 共用同一套诊断与 Markdown 报告；UI-13.5a 进一步提供 New UI/TUI/CLI/Agent Tool 共用的 typed 脱敏 ZIP，先预览固定文件集与 digest，再用精确 Snapshot 摘要原子写入平台状态目录。
