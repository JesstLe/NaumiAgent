...
| 02 对话时间线与输入器 | 多行编辑、原子 paste、受限高度 Composer、会话草稿恢复、`follow_tail`、语义未读计数、resize 消息锚点、用户消息发送确认/去重/失败重试、uncertain outbox 恢复、`Ctrl+R` 项目历史搜索、UI state v3 迁移 | 斜杠命令补全键盘选择 |
