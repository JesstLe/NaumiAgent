...
actor FakeWorkbenchAPIProvider: WorkbenchAPIProviding, WorkbenchRouteTemplateConfiguring {
    ...
    var fetchApprovalHook: (@Sendable () async -> Void)?
    ...
    func fetchChatEnvironment(
        ...
        fetchChatEnvironmentCallCount += 1
        guard let chatEnvironmentResult else { throw .invalidResponse }
        return try chatEnvironmentResult.get()
    }
    ...
    func fetchValidationRuns(
        ...
        await recordPreWarmRequest()
        if let fetchValidationRunsHook {
            await fetchValidationRunsHook()
        }
    ...
    func fetchContextSnapshot(sessionID: String, snapshotID: String) async throws(APIError) -> ContextSnapshotDTO {
        if let fetchContextSnapshotHook {
            await fetchContextSnapshotHook()
        }
    ...
    func fetchApproval(sessionID: String, approvalID: String) async throws(APIError) -> ApprovalDTO {
        if let fetchApprovalHook {
            await fetchApprovalHook()
        }
    ...
    func fetchFailures(
        ...
        await recordPreWarmRequest()
        if let fetchFailuresHook {
            await fetchFailuresHook()
        }
    ...
    func fetchFailure(sessionID: String, failureID: String) async throws(APIError) -> FailureDTO {
        if let fetchFailureHook {
            await fetchFailureHook()
        }
    ...
    func fetchLeases(
        ...
        await recordPreWarmRequest()
        if let fetchLeasesHook {
            await fetchLeasesHook()
        }
    ...
    func fetchWorktree(sessionID: String, name: String) async throws(APIError) -> WorktreeDTO {
        if let fetchWorktreeHook {
            await fetchWorktreeHook()
        }
    ...
    func fetchMissions(
        ...
        await recordPreWarmRequest()
        if let fetchMissionsHook {
            await fetchMissionsHook()
        }
    ...
    func fetchAgentProfile(sessionID: String, agentID: String) async throws(APIError) -> AgentProfileDTO {
        if let fetchAgentProfileHook {
            await fetchAgentProfileHook()
        }
    ...
    func releaseLeaseWithSnapshot(sessionID: String, leaseID: String) async throws(APIError) -> LeaseSnapshotDTO {
        ...
        if let releaseLeaseWithSnapshotHook {
            await releaseLeaseWithSnapshotHook()
        }
    ...
    func attachIssueWithSnapshot(
        ...
        if let attachIssueWithSnapshotHook {
            await attachIssueWithSnapshotHook()
        }
    ...
    func fetchIntentLocks(
        ...
        await recordPreWarmRequest()
        if let fetchIntentLocksHook {
            await fetchIntentLocksHook()
        }
    ...
    func fetchDecisions(
        ...
        await recordPreWarmRequest()
        if let fetchDecisionsHook {
            await fetchDecisionsHook()
        }
...
final class DaemonControllerTests {
    ...
    @Test @MainActor func refreshConnectionFailureClearsStaleDaemonMetadata() async throws {
        let appState = AppState()
        appState.connectionState = .connected
    ...
    @Test @MainActor func refreshConnectionKeepsSnapshotIssuesWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotIssue = makeIssue(taskID: "task-snapshot", missionID: "mission-snapshot")
        let snapshot = WorkbenchSnapshotDTO(
            sessionID: "sess-existing",
            missions: [],
            tasks: [],
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setIssuesResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.issues == [snapshotIssue])
    }

    @Test @MainActor func refreshConnectionKeepsSnapshotAgentProfilesWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotProfile = makeAgentProfile(
            id: "agent-snapshot",
            sessionID: "sess-existing",
            status: "busy"
        )
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setAgentProfilesResult(.failure(.httpStatus(502)))
    ...
    @Test @MainActor func refreshConnectionKeepsSnapshotEventsWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotEvent = makeEvent(
            id: "event-snapshot",
            type: "failure.created",
            subjectID: "failure-snapshot"
        )
        let snapshot = WorkbenchSnapshotDTO(
            sessionID: "sess-existing",
            missions: [],
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setEventsResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.timelineEvents == [snapshotEvent])
    }

    @Test @MainActor func refreshConnectionKeepsSnapshotIntentLocksWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotMission = makeMission(id: "mission-snapshot", sessionID: "sess-existing")
        let snapshotLock = makeIntentLock(id: "lock-snapshot", missionID: "mission-snapshot")
        let snapshot = WorkbenchSnapshotDTO(
            sessionID: "sess-existing",
            missions: [snapshotMission],
            intentLocks: [snapshotLock],
            tasks: [],
            issues: [],
            failures: [],
            events: []
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setFetchIntentLocksResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.intentLocks == [snapshotLock])
    }

    @Test @MainActor func refreshConnectionKeepsSnapshotDecisionsWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotMission = makeMission(id: "mission-snapshot", sessionID: "sess-existing")
        let snapshotDecision = makeDecision(id: "decision-snapshot", missionID: "mission-snapshot")
        let snapshot = WorkbenchSnapshotDTO(
            sessionID: "sess-existing",
            missions: [snapshotMission],
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setFetchDecisionsResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.decisions == [snapshotDecision])
    }
    ...
    @Test @MainActor func refreshConnectionKeepsSnapshotApprovalsWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotApproval = makeApproval(
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setApprovalsResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.approvals == [snapshotApproval])
    }

    @Test @MainActor func refreshConnectionKeepsSnapshotLeasesWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotLease = makeLease(
            id: "lease-snapshot",
            taskID: "task-snapshot",
            state: "active"
        )
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setLeasesResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.leases == [snapshotLease])
    }

    @Test @MainActor func refreshConnectionKeepsSnapshotWorktreesWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotWorktree = makeWorktree(
            name: "wt-snapshot",
            taskID: "task-snapshot",
            status: "dirty"
        )
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setWorktreesResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.worktrees == [snapshotWorktree])
    }

    @Test @MainActor func refreshConnectionKeepsSnapshotContextSnapshotsWhenPreWarmFails() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-existing"

        let api = FakeWorkbenchAPIProvider()
        let snapshotContext = makeContextSnapshot(
            id: "ctx-snapshot",
            taskID: "task-snapshot",
            health: "stale"
        )
        let snapshot = WorkbenchSnapshotDTO(
            ...
            failures: [],
            events: [],
            contextSnapshots: [snapshotContext]
        )
        ...
        await api.setSnapshotResult(.success(snapshot))
        await configureWorkbenchListResults(for: api, sessionID: "sess-existing")
        await api.setContextSnapshotsResult(.failure(.httpStatus(502)))
        ...
        #expect(appState.snapshot == snapshot)
        #expect(appState.lastError == .httpStatus(502))
        #expect(appState.contextSnapshots == [snapshotContext])
    }
    ...
    @Test @MainActor func eventStreamConnectedAfterStaleSessionUnavailableStopsStreamAndKeepsStale() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.connectionState = .stale
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [])
        ...
        await eventProvider.emit(.connected(sessionID: "sess-missing"))
    ...
    @Test @MainActor func releaseLeaseIgnoresSnapshotAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherLease = makeLease(id: "lease-other", taskID: "task-other", state: "active")
        let otherEvent = makeEvent(id: "evt-other", type: "lease.released", subjectID: "lease-other")
        let otherSnapshot = makeSnapshot(sessionID: "sess-other", lease: otherLease)

        let staleLease = makeLease(id: "lease-stale", taskID: "task-stale", state: "released")
        let staleSnapshot = makeSnapshot(sessionID: "sess-001", lease: staleLease)
        let staleEvent = makeEvent(id: "evt-stale", type: "lease.released", subjectID: "lease-stale")

        let api = FakeWorkbenchAPIProvider()
        await api.setReleaseLeaseWithSnapshotResult(.success(
            LeaseSnapshotDTO(lease: staleLease, snapshot: staleSnapshot)
        ))
        await api.setLeasesResult(.success(
            LeasesDTO(leases: [staleLease], state: nil, taskID: nil, agentID: nil, limit: 50)
        ))
        await api.setEventsResult(.success(WorkbenchEventsDTO(events: [staleEvent], limit: 50)))
        await api.setReleaseLeaseWithSnapshotHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.snapshot = otherSnapshot
                appState.leases = [otherLease]
        ...
        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.releaseLease(leaseID: "lease-stale")
    ...
    @Test @MainActor func releaseLeaseIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherLease = makeLease(id: "lease-other", taskID: "task-other", state: "active")
        let otherSnapshot = makeSnapshot(sessionID: "sess-other", lease: otherLease)
    ...
    @Test @MainActor func refreshMissionsIgnoresMissionsAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherMission = makeMission(id: "mission-other", sessionID: "sess-other")
        let staleMission = makeMission(id: "mission-stale", sessionID: "sess-001")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setMissionsResult(.success(MissionsDTO(
            missions: [staleMission],
            status: nil,
            limit: 50
        )))
        await api.setFetchMissionsHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.missions = [otherMission]
            }
        ...
        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.refreshMissions(limit: 50)
    ...
    @Test @MainActor func refreshMissionsIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherMission = makeMission(id: "mission-other", sessionID: "sess-other")

        let api = FakeWorkbenchAPIProvider()
        await api.setMissionsResult(.failure(.sessionUnavailable))
        await api.setFetchMissionsHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.missions = [otherMission]
    ...
    @Test @MainActor func refreshMissionsSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
    ...
    @Test @MainActor func attachIssueIgnoresSnapshotAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherIssue = makeIssue(taskID: "task-other", missionID: "mission-other")
        let otherEvent = makeEvent(id: "evt-other", type: "issue.attached", subjectID: "task-other")
        ...
        let staleEvent = makeEvent(id: "evt-stale", type: "issue.attached", subjectID: "task-stale")
        ...
        await api.setEventsResult(.success(WorkbenchEventsDTO(events: [staleEvent], limit: 50)))
        await api.setAttachIssueWithSnapshotHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.snapshot = otherSnapshot
                appState.issues = [otherIssue]
    ...
    @Test @MainActor func attachIssueIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherIssue = makeIssue(taskID: "task-other", missionID: "mission-other")
        let otherSnapshot = makeSnapshot(sessionID: "sess-other", issues: [otherIssue])
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setAttachIssueWithSnapshotResult(.failure(.sessionUnavailable))
        await api.setAttachIssueWithSnapshotHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.snapshot = otherSnapshot
    ...
    @Test @MainActor func attachIssueSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [])
        seedWorkbenchLists(appState)
    ...
    @Test @MainActor func refreshEventsSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshValidationRunsIgnoresRunsAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setValidationRunsResult(.success(ValidationRunsDTO(
            validationRuns: [staleRun],
            taskID: nil,
            status: nil,
            limit: 50
        )))
        await api.setFetchValidationRunsHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
    ...
    @Test @MainActor func refreshValidationRunsIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherRun = makeValidationRun(id: "run-other", taskID: "task-other", status: "passed")

        let api = FakeWorkbenchAPIProvider()
        await api.setValidationRunsResult(.failure(.sessionUnavailable))
        await api.setFetchValidationRunsHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.validationRuns = [otherRun]
    ...
    @Test @MainActor func refreshValidationRunsSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshContextSnapshotsSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshApprovalsSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshFailuresIgnoresFailuresAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setFailuresResult(.success(FailuresDTO(
            failures: [staleFailure],
            taskID: nil,
            status: nil,
            limit: 50
        )))
        await api.setFetchFailuresHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
    ...
    @Test @MainActor func refreshFailuresIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherFailure = makeFailure(id: "failure-other", taskID: "task-other", status: "open")

        let api = FakeWorkbenchAPIProvider()
        await api.setFailuresResult(.failure(.sessionUnavailable))
        await api.setFetchFailuresHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.failures = [otherFailure]
    ...
    @Test @MainActor func refreshFailuresSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshIssuesSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshLeasesIgnoresLeasesAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherLease = makeLease(id: "lease-other", taskID: "task-other", state: "active")
        let staleLease = makeLease(id: "lease-stale", taskID: "task-stale", state: "active")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setLeasesResult(.success(LeasesDTO(
            leases: [staleLease],
            state: nil,
            taskID: nil,
            agentID: nil,
    ...
    @Test @MainActor func refreshLeasesIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherLease = makeLease(id: "lease-other", taskID: "task-other", state: "active")

        let api = FakeWorkbenchAPIProvider()
        await api.setLeasesResult(.failure(.sessionUnavailable))
        await api.setFetchLeasesHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.leases = [otherLease]
    ...
    @Test @MainActor func refreshLeasesSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshWorktreesSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func keepWorktreeSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setKeepWorktreeWithSnapshotResult(.failure(.sessionUnavailable))
    ...
    @Test @MainActor func refreshIntentLocksIgnoresLocksAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherLock = makeIntentLock(id: "lock-other", missionID: "mission-other")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setFetchIntentLocksResult(.success(IntentLocksDTO(
            intentLocks: [staleLock],
            missionID: "mission-001"
        )))
        await api.setFetchIntentLocksHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.intentLocks = [otherLock]
            }
    ...
    @Test @MainActor func refreshIntentLocksIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherLock = makeIntentLock(id: "lock-other", missionID: "mission-other")

        let api = FakeWorkbenchAPIProvider()
        await api.setFetchIntentLocksResult(.failure(.sessionUnavailable))
        await api.setFetchIntentLocksHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.intentLocks = [otherLock]
    ...
    @Test @MainActor func refreshIntentLocksSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func refreshDecisionsIgnoresDecisionsAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherDecision = makeDecision(id: "decision-other", missionID: "mission-other")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setFetchDecisionsResult(.success(DecisionsDTO(
            decisions: [staleDecision],
            missionID: "mission-001"
        )))
        await api.setFetchDecisionsHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.decisions = [otherDecision]
            }
    ...
    @Test @MainActor func refreshDecisionsIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherDecision = makeDecision(id: "decision-other", missionID: "mission-other")

        let api = FakeWorkbenchAPIProvider()
        await api.setFetchDecisionsResult(.failure(.sessionUnavailable))
        await api.setFetchDecisionsHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.decisions = [otherDecision]
    ...
    @Test @MainActor func refreshDecisionsSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadIntentLockSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadApprovalIgnoresApprovalAfterSelectedSessionChanges() async throws {
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setApprovalResult(.success(staleApproval))
        await api.setFetchApprovalHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedApproval = otherApproval
            }
        }

        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.loadApproval(approvalID: "approval-stale")

        #expect(appState.selectedSessionID == "sess-other")
        #expect(appState.selectedApproval == otherApproval)
        #expect(appState.lastError == nil)
    }

    @Test @MainActor func loadApprovalIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherApproval = makeApproval(id: "approval-other", missionID: "mission-other", state: "waiting")

        let api = FakeWorkbenchAPIProvider()
        await api.setApprovalResult(.failure(.sessionUnavailable))
        await api.setFetchApprovalHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedApproval = otherApproval
    ...
    @Test @MainActor func loadApprovalSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadEventSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadContextSnapshotIgnoresSnapshotAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherSnapshot = makeContextSnapshot(id: "context-other", taskID: "task-other", health: "good")
        let staleSnapshot = makeContextSnapshot(id: "context-stale", taskID: "task-stale", health: "stale")
        appState.selectedContextSnapshot = nil

        let api = FakeWorkbenchAPIProvider()
        await api.setContextSnapshotResult(.success(staleSnapshot))
        await api.setFetchContextSnapshotHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedContextSnapshot = otherSnapshot
            }
        }

        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.loadContextSnapshot(snapshotID: "context-stale")

        #expect(appState.selectedSessionID == "sess-other")
        #expect(appState.selectedContextSnapshot == otherSnapshot)
        #expect(appState.lastError == nil)
    }

    @Test @MainActor func loadContextSnapshotIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherSnapshot = makeContextSnapshot(id: "context-other", taskID: "task-other", health: "good")
    ...
    @Test @MainActor func loadFailureIgnoresFailureAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherFailure = makeFailure(id: "failure-other", taskID: "task-other", status: "open")
        let staleFailure = makeFailure(id: "failure-stale", taskID: "task-stale", status: "open")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setFailureResult(.success(staleFailure))
        await api.setFetchFailureHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedFailure = otherFailure
            }
        }

        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.loadFailure(failureID: "failure-stale")

        #expect(appState.selectedSessionID == "sess-other")
        #expect(appState.selectedFailure == otherFailure)
        #expect(appState.lastError == nil)
    }

    @Test @MainActor func loadFailureIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherFailure = makeFailure(id: "failure-other", taskID: "task-other", status: "open")

        let api = FakeWorkbenchAPIProvider()
        await api.setFailureResult(.failure(.sessionUnavailable))
        await api.setFetchFailureHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedFailure = otherFailure
    ...
    @Test @MainActor func loadFailureSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadIssueSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadLeaseSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadWorktreeIgnoresWorktreeAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherWorktree = makeWorktree(name: "wt-other", taskID: "task-other", status: "active")
        let staleWorktree = makeWorktree(name: "wt-stale", taskID: "task-stale", status: "active")
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setWorktreeResult(.success(staleWorktree))
        await api.setFetchWorktreeHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedWorktree = otherWorktree
            }
        }

        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.loadWorktree(name: "wt-stale")

        #expect(appState.selectedSessionID == "sess-other")
        #expect(appState.selectedWorktree == otherWorktree)
        #expect(appState.lastError == nil)
    }

    @Test @MainActor func loadWorktreeIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherWorktree = makeWorktree(name: "wt-other", taskID: "task-other", status: "active")

        let api = FakeWorkbenchAPIProvider()
        await api.setWorktreeResult(.failure(.sessionUnavailable))
        await api.setFetchWorktreeHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedWorktree = otherWorktree
    ...
    @Test @MainActor func loadWorktreeSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadMissionSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func loadAgentProfileIgnoresProfileAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        ...
        let api = FakeWorkbenchAPIProvider()
        await api.setAgentProfileResult(.success(staleProfile))
        await api.setFetchAgentProfileHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedAgentProfile = otherProfile
            }
        }

        let controller = DaemonController(appState: appState, apiProvider: api)
        await controller.loadAgentProfile(agentID: "agent-stale")

        #expect(appState.selectedSessionID == "sess-other")
        #expect(appState.selectedAgentProfile == otherProfile)
        #expect(appState.lastError == nil)
    }

    @Test @MainActor func loadAgentProfileIgnoresSessionUnavailableAfterSelectedSessionChanges() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-001"
        let otherProfile = makeAgentProfile(id: "agent-other", sessionID: "sess-other", status: "idle")

        let api = FakeWorkbenchAPIProvider()
        await api.setAgentProfileResult(.failure(.sessionUnavailable))
        await api.setFetchAgentProfileHook {
            await MainActor.run {
                appState.selectedSessionID = "sess-other"
                appState.selectedAgentProfile = otherProfile
    ...
    @Test @MainActor func loadAgentProfileSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [
            makeMission(id: "mission-stale", sessionID: "sess-missing")
    ...
    @Test @MainActor func resolveApprovalSessionUnavailableClearsSelectedSessionAndSessionState() async throws {
        let appState = AppState()
        appState.selectedSessionID = "sess-missing"
        appState.snapshot = makeSnapshot(sessionID: "sess-missing", missions: [])
        seedWorkbenchLists(appState)
    ...
    @Test @MainActor func refreshChatMessagesAlsoRestoresPersistedRuns() async throws {
        ...
        let api = FakeWorkbenchAPIProvider()
        let environment = ChatEnvironmentDTO(
            sessionID: "sess-001",
            workspaceRoot: "/repo",
            workspaceName: "repo",
        ...
        await api.setChatEnvironmentResult(.success(environment))
...
extension FakeWorkbenchAPIProvider {
    ...
    fileprivate func setChatEnvironmentResult(
        _ result: Result<ChatEnvironmentDTO, APIError>
    ) {
        chatEnvironmentResult = result
    }
    ...
    fileprivate func setFetchValidationRunsHook(_ hook: (@Sendable () async -> Void)?) {
        fetchValidationRunsHook = hook
    }
    ...
    fileprivate func setFetchContextSnapshotHook(_ hook: (@Sendable () async -> Void)?) {
        fetchContextSnapshotHook = hook
    }
    ...
    fileprivate func setFetchApprovalHook(_ hook: (@Sendable () async -> Void)?) {
        fetchApprovalHook = hook
    }
    ...
    fileprivate func setFetchFailuresHook(_ hook: (@Sendable () async -> Void)?) {
        fetchFailuresHook = hook
    }
    ...
    fileprivate func setFetchFailureHook(_ hook: (@Sendable () async -> Void)?) {
        fetchFailureHook = hook
    }
    ...
    fileprivate func setFetchWorktreeHook(_ hook: (@Sendable () async -> Void)?) {
        fetchWorktreeHook = hook
    }
    ...
    fileprivate func setFetchMissionsHook(_ hook: (@Sendable () async -> Void)?) {
        fetchMissionsHook = hook
    }
    ...
    fileprivate func setAttachIssueWithSnapshotHook(_ hook: (@Sendable () async -> Void)?) {
        attachIssueWithSnapshotHook = hook
    }
    ...
    fileprivate func setFetchAgentProfileHook(_ hook: (@Sendable () async -> Void)?) {
        fetchAgentProfileHook = hook
    }
    ...
    fileprivate func setReleaseLeaseWithSnapshotHook(_ hook: (@Sendable () async -> Void)?) {
        releaseLeaseWithSnapshotHook = hook
    }
    ...
    fileprivate func setFetchIntentLocksHook(_ hook: (@Sendable () async -> Void)?) {
        fetchIntentLocksHook = hook
    }
    ...
    fileprivate func setFetchDecisionsHook(_ hook: (@Sendable () async -> Void)?) {
        fetchDecisionsHook = hook
    }
