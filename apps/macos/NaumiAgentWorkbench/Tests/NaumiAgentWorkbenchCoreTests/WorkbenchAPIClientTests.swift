...
final class WorkbenchAPIClientTests {
    ...
    @Test func fetchEventUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let eventID = "event/时间线"
        let json = Data(
            """
            {"id":"event/时间线","session_id":"sess/中文","type":"validation.failed","actor":"Validation-Agent","subject_id":"task-001","payload":{"run_id":"run-001"},"timestamp":"2026-06-27T10:20:00"}
            """.utf8
        )

        MockURLProtocol.requestHandler = { request in
            guard request.url?.absoluteString == "http://127.0.0.1:8765/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/audit-events/event%2F%E6%97%B6%E9%97%B4%E7%BA%BF" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "GET" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        let client = makeClient(routeTemplates: [
            "event": "/workbench-v2/sessions/{session_id}/audit-events/{event_id}",
        ])
        let event = try await client.fetchEvent(sessionID: sessionID, eventID: eventID)

        #expect(event.id == eventID)
        #expect(event.sessionID == sessionID)
        #expect(event.type == "validation.failed")
        #expect(event.payload == ["run_id": .string("run-001")])
    }
    ...
    @Test func removeWorktreeUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let worktreeName = "wt/清理"
        let json = Data(
            """
            {"name":"wt/清理","discard_changes":false,"message":"已删除 worktree：wt/清理"}
            """.utf8
        )
        ...
        MockURLProtocol.requestHandler = { request in
            ...
            guard components?.percentEncodedPath == "/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/worktrees/wt%2F%E6%B8%85%E7%90%86",
                  query["discard_changes"] == "false" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "DELETE" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        #expect(result.name == worktreeName)
        #expect(!result.discardChanges)
        #expect(result.message == "已删除 worktree：wt/清理")
    }
    ...
    @Test func fetchMissionUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let missionID = "mission/总览"
        let json = Data(
            """
            {"id":"mission/总览","session_id":"sess/中文","title":"模板 Mission 详情","goal":"从模板路径加载 Mission 详情","status":"planning","created_at":"2026-06-27T06:00:00","updated_at":"2026-06-27T06:10:00"}
            """.utf8
        )

        MockURLProtocol.requestHandler = { request in
            guard request.url?.absoluteString == "http://127.0.0.1:8765/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/missions/mission%2F%E6%80%BB%E8%A7%88" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "GET" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        let client = makeClient(routeTemplates: [
            "mission": "/workbench-v2/sessions/{session_id}/missions/{mission_id}",
        ])
        let mission = try await client.fetchMission(sessionID: sessionID, missionID: missionID)

        #expect(mission.id == missionID)
        #expect(mission.sessionID == sessionID)
        #expect(mission.title == "模板 Mission 详情")
        #expect(mission.goal == "从模板路径加载 Mission 详情")
    }
    ...
    @Test func expireLeasesUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let responseJSON = Data(
            """
            {"expired":[{"id":"lease-过期","session_id":"sess/中文","task_id":"task-001","agent_id":"agent-001","state":"expired","expires_at":"2026-06-27T08:00:00","worktree_name":"wt-001","created_at":"2026-06-27T06:00:00","updated_at":"2026-06-27T06:30:00"}]}
            """.utf8
        )

        MockURLProtocol.requestHandler = { request in
            guard request.url?.absoluteString == "http://127.0.0.1:8765/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/leases/expire" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "POST" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        #expect(result.expired.map(\.id) == ["lease-过期"])
        #expect(result.expired.first?.state == "expired")
    }
    ...
    @Test func createDecisionUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let missionID = "mission/治理"
        let decisionJSON = Data(
            """
            {"id":"decision-template","session_id":"sess/中文","mission_id":"mission/治理","kind":"policy","title":"保留人工审批","content":"高风险任务必须经过人工审批","actor":"Planner-Agent","created_at":"2026-06-27T06:00:00"}
            """.utf8
        )

        MockURLProtocol.requestHandler = { request in
            guard request.url?.absoluteString == "http://127.0.0.1:8765/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/missions/mission%2F%E6%B2%BB%E7%90%86/decisions" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "POST" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        #expect(decision.id == "decision-template")
        #expect(decision.sessionID == sessionID)
        #expect(decision.missionID == missionID)
    ...
    @Test func resolveApprovalUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let approvalID = "approval/人工审批"
        let approvalJSON = Data(
            """
            {"id":"approval/人工审批","session_id":"sess/中文","mission_id":"mission-001","task_id":"task-001","state":"approved","title":"允许发布","detail":"验证已通过","requester":"Agent-A","reviewer":"Human","decision_note":"证据充分","created_at":"2026-06-27T06:00:00","updated_at":"2026-06-27T06:00:01"}
            """.utf8
        )

        MockURLProtocol.requestHandler = { request in
            guard request.url?.absoluteString == "http://127.0.0.1:8765/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/approvals/approval%2F%E4%BA%BA%E5%B7%A5%E5%AE%A1%E6%89%B9/resolve" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "POST" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        #expect(approval.sessionID == sessionID)
        #expect(approval.state == "approved")
        #expect(approval.decisionNote == "证据充分")
    }
    ...
    @Test func fetchIntentLockUsesConfiguredRouteTemplate() async throws {
        let sessionID = "sess/中文"
        let missionID = "mission/治理"
        let lockID = "lock/模板"
        let json = Data(
            """
            {"id":"lock/模板","session_id":"sess/中文","mission_id":"mission/治理","rule":"模板路径加载详情","blocked_paths":["src/core"],"allowed_paths":["docs/adr"],"require_proposal_for_risk":"critical","active":true,"created_at":"2026-06-27T06:00:00"}
            """.utf8
        )

        MockURLProtocol.requestHandler = { request in
            guard request.url?.absoluteString == "http://127.0.0.1:8765/api/v1/workbench-v2/sessions/sess%2F%E4%B8%AD%E6%96%87/missions/mission%2F%E6%B2%BB%E7%90%86/intent-locks/lock%2F%E6%A8%A1%E6%9D%BF" else {
                fatalError("Unexpected URL: \(String(describing: request.url))")
            }
            guard request.httpMethod == "GET" else {
                fatalError("Unexpected method: \(String(describing: request.httpMethod))")
        ...
        let client = makeClient(routeTemplates: [
            "intent_lock": "/workbench-v2/sessions/{session_id}/missions/{mission_id}/intent-locks/{lock_id}",
        ])
        let lock = try await client.fetchIntentLock(sessionID: sessionID, missionID: missionID, lockID: lockID)
