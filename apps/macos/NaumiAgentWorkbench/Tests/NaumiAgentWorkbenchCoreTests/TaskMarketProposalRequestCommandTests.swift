...
struct TaskMarketProposalRequestCommandTests {

    @Test func commandBuildsProposalDecisionFromSelectedIssueAndBid() throws {
        let command = try #require(TaskMarketProposalRequestCommand(
        ...
        #expect(command.title == "提案请求：实现 API Client")
        ...
        #expect(command.content.contains("request_reason: Human requested a concrete proposal before assigning the bid."))
    }

    @Test func englishTitleUsesProposalRequestPrefix() throws {
        let command = try #require(TaskMarketProposalRequestCommand(
            issue: issue(taskID: "task-123"),
        ...
        #expect(command.title == "Proposal Request: 实现 API Client")
    }

    @Test func commandIsNilWhenMissionIDIsEmpty() {
        #expect(TaskMarketProposalRequestCommand(
            issue: issue(taskID: "task-123"),
            bid: bid(agent: "Backend-Agent"),
            missionID: "   ",
            locale: .zhCN
