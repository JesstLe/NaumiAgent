...
public actor WorkbenchAPIClient: Sendable, WorkbenchAPIProviding, WorkbenchRouteTemplateConfiguring, ChatStreamingProviding, ChatRunProviding, ChatEnvironmentProviding {
    ...
    public func fetchChatRuns(
        sessionID: String,
        limit: Int = 50
    ) async throws(APIError) -> ChatRunsDTO {
        let path = encodePath("sessions", sessionID, "runs")
        return try await get(
            path: path,
    ...
    public func fetchApprovals(
        ...
        let path = try routePath(
            named: "approvals",
            replacements: ["session_id": sessionID],
            fallback: encodePath("workbench", "sessions", sessionID, "approvals")
        )
        return try await get(
            path: path,
            queryItems: queryItems
    ...
    public func fetchFailures(
        ...
        let path = try routePath(
            named: "failures",
            replacements: ["session_id": sessionID],
            fallback: encodePath("workbench", "sessions", sessionID, "failures")
        )
        return try await get(
            path: path,
            queryItems: queryItems
    ...
    public func fetchIssues(
        ...
        let path = try routePath(
            named: "issues",
            replacements: ["session_id": sessionID],
            fallback: encodePath("workbench", "sessions", sessionID, "issues")
        )
        return try await get(
            path: path,
            queryItems: queryItems
    ...
    public func fetchIssue(sessionID: String, taskID: String) async throws(APIError) -> IssueDTO {
        let path = try routePath(
            ...
            fallback: encodePath("workbench", "sessions", sessionID, "issues", taskID)
        )
        return try await get(path: path)
    }
    ...
    public func fetchLeases(
        ...
        let path = try routePath(
            named: "leases",
            replacements: ["session_id": sessionID],
            fallback: encodePath("workbench", "sessions", sessionID, "leases")
        )
        return try await get(
            path: path,
            queryItems: queryItems
    ...
    public func fetchLease(sessionID: String, leaseID: String) async throws(APIError) -> LeaseDTO {
        let path = try routePath(
            ...
            fallback: encodePath("workbench", "sessions", sessionID, "leases", leaseID)
        )
        return try await get(path: path)
    }
    ...
    public func fetchWorktrees(
        ...
        let path = try routePath(
            named: "worktrees",
            replacements: ["session_id": sessionID],
            fallback: encodePath("workbench", "sessions", sessionID, "worktrees")
        )
        return try await get(
            path: path,
            queryItems: queryItems
    ...
    public func fetchAgentProfiles(
        ...
        let path = try routePath(
            named: "agents",
            replacements: ["session_id": sessionID],
            fallback: encodePath("workbench", "sessions", sessionID, "agents")
        )
        return try await get(
            path: path,
            queryItems: queryItems
    ...
    public func releaseLease(sessionID: String, leaseID: String) async throws(APIError) -> LeaseDTO {
        let path = try routePath(
            ...
            fallback: encodePath("workbench", "sessions", sessionID, "leases", leaseID, "release")
        )
