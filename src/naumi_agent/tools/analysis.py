...
def _build_eval_baseline(files: list[Path]) -> EvalBaseline:
    ...
    for file in files:
        if file.suffix != ".py":
            continue
        try:
            tree = _ast.parse(file.read_text(encoding="utf-8", errors="replace"))
        except SyntaxError:
            continue
        ...
        for node in tree.body:
            if isinstance(node, (_ast.FunctionDef, _ast.AsyncFunctionDef)):
                ...
                for child in node.body:
                    if not isinstance(child, (_ast.FunctionDef, _ast.AsyncFunctionDef)):
                        continue
    ...
    test_code = _render_eval_baseline(functions, classes, target_files)
    return EvalBaseline(
        test_code=test_code,
...
def _render_eval_baseline(
    ...
    class_rows = [
        (cls.file_path, cls.module_name, cls.class_name)
        for cls in classes
    ]
...
def _format_eval_baseline(scan_evidence: str, baseline: EvalBaseline) -> str:
    return (
        "## Eval 静态扫描\n"
        f"{scan_evidence}\n\n"
        "## EDD Baseline Pytest\n"
...
def _build_heal_report(
    error_log: str,
    scan_evidence: str = "",
    files: list[Path] | None = None,
) -> str:
    error_type, error_message = _extract_error_summary(error_log)
    frames = _extract_traceback_frames(error_log)
    ...
    lines.append("")
    lines.append("## 最小修复方向")
    lines.extend(_heal_repair_guidance(error_type, error_message))
    lines.append("")
    lines.append("## 回归验证建议")
...
def _extract_error_summary(error_log: str) -> tuple[str, str]:
    candidates = [
        line.strip()
...
def _extract_traceback_frames(error_log: str) -> list[tuple[str, int, str]]:
...
def _heal_repair_guidance(error_type: str, error_message: str) -> list[str]:
...
def _build_mcts_decision_report(problem: str, scan_evidence: str = "") -> str:
    complexity = _extract_mcts_complexity(scan_evidence)
...
def _extract_mcts_complexity(scan_evidence: str) -> str:
...
def _build_route_report(
    ...
    experts = _select_route_experts(task, source_text)
    lines = [
        "## MoE 确定性专家路由",
        f"- 任务：{task}",
        f"- 专家数：{len(experts)}",
    ...
    for expert in experts:
        lines.extend(
            [
                ...
                f"- Recommendation: {_route_domain_recommendation(expert.domain)}",
                f"- Concern: {_route_domain_concern(expert.domain)}",
            ]
        )
    lines.extend(
        [
    ...
    for idx, expert in enumerate(experts, 1):
        lines.append(
            f"{idx}. [{expert.domain}] 先完成 `{expert.focus}` 的最小可验证改动。"
        )
    lines.extend(
        [
            ...
            "",
            "## Resource Estimate",
            f"- Complexity: {_route_complexity(experts)}",
...
def _select_route_experts(task: str, source_text: str = "") -> list[RouteExpert]:
    combined = f"{task}\n{source_text}".lower()
    ...
    return [
        RouteExpert(
            domain=domain,
            focus=_route_domain_focus(domain),
...
def _route_domain_focus(domain: str) -> str:
    return {
...
def _route_domain_recommendation(domain: str) -> str:
    return {
...
def _route_domain_concern(domain: str) -> str:
    return {
...
def _route_conflicts(experts: list[RouteExpert]) -> list[str]:
...
def _route_complexity(experts: list[RouteExpert]) -> str:
    count = len(experts)
    if count >= 5:
        return "XL"
    if count == 4:
...
def _build_jit_baseline(task: str, context: str = "") -> JITBaseline:
    expression = _extract_arithmetic_expression(task)
    if expression:
        try:
            result = _safe_eval_arithmetic(expression)
            script = _render_jit_arithmetic_script(task, context, expression, result)
            output = f"EXPRESSION={expression}\nRESULT={result}\nSTATUS=verified"
            return JITBaseline(script=script, execution_output=output, verified=True)
        except Exception as exc:
            script = _render_jit_triage_script(task, context, f"算术表达式拒绝执行：{exc}")
            output = f"STATUS=needs_manual_verification\nREASON={exc}"
            return JITBaseline(script=script, execution_output=output, verified=False)

    script = _render_jit_triage_script(
        task,
        context,
...
def _extract_arithmetic_expression(task: str) -> str:
...
def _safe_eval_arithmetic(expression: str) -> int | float:
    ...
    def eval_node(node: _ast.AST) -> int | float:
        if isinstance(node, _ast.Expression):
            return eval_node(node.body)
        if isinstance(node, _ast.Constant) and isinstance(node.value, int | float):
            return node.value
        if isinstance(node, _ast.BinOp) and type(node.op) in operators:
            left = eval_node(node.left)
            right = eval_node(node.right)
            if isinstance(node.op, _ast.Pow) and abs(right) > 10:
                raise ValueError("幂指数过大")
            return operators[type(node.op)](left, right)
        if isinstance(node, _ast.UnaryOp) and type(node.op) in operators:
...
def _render_jit_arithmetic_script(
    task: str,
    context: str,
    expression: str,
    result: int | float,
...
def _render_jit_triage_script(task: str, context: str, reason: str) -> str:
    return f'''\
...
def _format_jit_baseline(scan_evidence: str, baseline: JITBaseline) -> str:
