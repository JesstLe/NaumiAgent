    ...
    try:
        ...
        if action == "discover-metric":
            ...
            await _run_tool_slash_command(
                ...
                parse_args=lambda _arg: {"comparison_id": parts[1]},
                arg="",
            )
            return
        ...
        if action == "reward-hacking":
            ...
            return
        if action == "decision-state":
            if len(parts) != 2:
                raise ValueError("decision-state 需要一个 Decision Input ID。")
            artifact = await engine.evolution_decision_state_executor.execute(
                workspace_root=engine.workspace_root,
                decision_input_id=parts[1],
            )
        ...
        if action == "revalidation-validation-plan":
            ...
            return
        if action == "revalidation-runtime-contract":
            if len(parts) != 2:
                raise ValueError(
                    "revalidation-runtime-contract 需要一个 Validation Plan ID。"
                )
            view = (
                await engine.evolution_revalidation_runtime_contract_service.issue(
                    workspace_root=engine.workspace_root,
                    validation_plan_id=parts[1],
                )
            )
            console.print(
        ...
        if action == "stable-remote-readiness":
            ...
            return
        if action == "stable-remote-readiness-probe":
            if len(parts) in {3, 4} and parts[1] == "prepare":
                arguments = {
                    "action": "prepare",
                    "claim_receipt_id": parts[2],
                    "validity_seconds": int(parts[3]) if len(parts) == 4 else 180,
                }
            elif len(parts) == 3 and parts[1] == "execute-local":
                arguments = {
                    "action": "execute-local",
                    "challenge_base64": parts[2],
                }
            elif len(parts) == 4 and parts[1] == "ingest":
                arguments = {
                    "action": "ingest",
                    "challenge_id": parts[2],
                ...
                raise ValueError(
                    "stable-remote-readiness-probe 需要 prepare <claim-id> "
                    "[validity-seconds]、execute-local <challenge-base64>、"
                    "ingest <challenge-id> <submission-base64> 或 "
                    "inspect <receipt-id>。"
                )
            await _run_tool_slash_command(
                engine,
        ...
        if action == "stable-remote-finalization-authorization":
            ...
            return
        if action == "stable-remote-finalization":
            if len(parts) == 3 and parts[1] == "prepare":
                arguments = {
                    "action": "prepare",
                    "authorization_id": parts[2],
                }
            elif len(parts) == 3 and parts[1] == "export":
                arguments = {"action": "export", "grant_id": parts[2]}
            elif len(parts) == 3 and parts[1] == "execute-local":
                arguments = {
                    "action": "execute-local",
                    "package_base64": parts[2],
                }
            elif len(parts) == 4 and parts[1] == "ingest":
                arguments = {
                    "action": "ingest",
        ...
        if action == "stable-remote-population-finalization":
            ...
            return
        if action == "stable-promotion-observation-contract":
            if len(parts) != 2:
                raise ValueError(
                    "stable-promotion-observation-contract 需要一个 "
            ...
            await _run_tool_slash_command(
                ...
                parse_args=lambda _arg: {"finalization_receipt_id": parts[1]},
                arg="",
            )
            return
        ...
        if action == "stable-promotion-observation-revisions":
            ...
            if len(parts) == 3 and parts[1] == "queue":
                ...
                return
            if len(parts) == 3 and parts[1] == "inspect-dispatch":
                identity = parts[2]
                field = (
                    "submission_id"
                    if identity.startswith("evstablepromrevsubmit_")
                    else "admission_id"
                )
                await _run_tool_slash_command(
                    engine,
                ...
                return
            if len(parts) == 2 and parts[1] in {"run-worker", "inspect-worker"}:
                await _run_tool_slash_command(
                    engine,
                    slash_command="/evolution",
                    tool_name=(
                    ...
                    parse_args=lambda _arg: {"action": parts[1]},
                    arg="",
                )
                return
            raise ValueError(
                ...
                "receive <payload-base64>、inspect <receipt-id>、queue "
        ...
        if action == "revalidation-rollback-outcome":
            if len(parts) != 2:
                raise ValueError(
                    "revalidation-rollback-outcome 需要一个 Rollback Request ID。"
                )
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
            ...
            return
        if action == "outcome-before-after":
            if len(parts) != 2:
                raise ValueError(
                    "outcome-before-after 需要一个 Rollback Request ID。"
                )
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
        ...
        if action == "outcome-verify-runtime":
            ...
            return
        if action == "outcome-verify-behavior":
            if len(parts) != 3:
                raise ValueError(
                    "outcome-verify-behavior 需要 Rollback Request ID 和 "
                    "原 Final Evaluation Comparison ID。"
                )
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
        ...
        if action == "outcome-behavior-coverage":
            ...
            return
        if action == "outcome-behavior-matrix":
            if len(parts) != 2:
                raise ValueError(
                    "outcome-behavior-matrix 需要一个 Rollback Request ID。"
                )
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
        ...
        if action == "outcome-admit-runtime":
            ...
            return
        if action == "outcome-assess-long-term":
            if len(parts) != 3:
                raise ValueError(
                    "outcome-assess-long-term 需要 Rollback Request ID 和 Runtime Subject ID。"
                )
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
        ...
        if action == "outcome-resolve-behavior":
            ...
            return
        if action == "outcome-dispatch-behavior":
            if len(parts) != 4:
                raise ValueError(
                    "outcome-dispatch-behavior 需要 Rollback Request ID、"
                    "Comparison ID 和 release channel。"
                )
            await _run_tool_slash_command(
                engine,
        ...
        if action == "outcome-claim-behavior":
            ...
            return
        if action == "outcome-deliver-behavior":
            if len(parts) < 3 or parts[1] not in {"prepare", "submit", "inspect"}:
                raise ValueError(
                    "outcome-deliver-behavior 需要 prepare、submit 或 inspect 子命令。"
                )
            operation = parts[1]
            if operation in {"prepare", "inspect"} and len(parts) != 3:
                raise ValueError(f"outcome-deliver-behavior {operation} 需要一个 ID。")
            if operation == "submit" and len(parts) != 4:
                raise ValueError(
                    "outcome-deliver-behavior submit 需要 Delivery ID 与 Worker signature。"
                )
            arguments = {"action": operation}
            if operation == "prepare":
                arguments["claim_id"] = parts[2]
            else:
                arguments["delivery_id"] = parts[2]
                if operation == "submit":
                    arguments["worker_signature_base64"] = parts[3]
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
            ...
            return
        if action == "outcome-authorize-behavior":
            if len(parts) < 3 or parts[1] not in {"prepare", "submit", "inspect"}:
                raise ValueError(
                    "outcome-authorize-behavior 需要 prepare、submit 或 inspect 子命令。"
                )
            operation = parts[1]
            if operation in {"prepare", "inspect"} and len(parts) != 3:
                raise ValueError(f"outcome-authorize-behavior {operation} 需要一个 ID。")
            if operation == "submit" and len(parts) != 4:
                raise ValueError(
                    "outcome-authorize-behavior submit 需要 Start Challenge ID "
                    "与 Worker signature。"
                )
            arguments = {"action": operation}
            if operation == "prepare":
                ...
                arguments["reference_id"] = parts[2]
            await _run_tool_slash_command(
                engine,
                slash_command="/evolution",
        ...
        if action == "capability-govern":
            if len(parts) != 2:
                raise ValueError("capability-govern 需要一个 Candidate ID。")
            view = await engine.evolution_capability_governance_service.decide(
                engine.workspace_root,
                candidate_id=parts[1],
            )
        ...
        if action == "capability-sandbox":
            if len(parts) != 2:
                raise ValueError("capability-sandbox 需要一个 Candidate ID。")
            view = await engine.evolution_capability_sandbox_request_service.prepare(
                engine.workspace_root,
                candidate_id=parts[1],
            )
        ...
        console.print(
            ...
            "/evolution outcome-deliver-behavior submit <delivery-id> "
        ...
        return
    except EvolutionRevalidationRuntimeContractError as exc:
        console.print(
            f"Evolution Fresh Runtime Contract 未签发：{exc}",
            style="yellow",
            markup=False,
        )
        return
...
async def _run_harness(engine: Any, arg: str) -> None:
    ...
    if (
        ...
        and len(parts) >= 4
        and parts[1].lower() == "sandbox"
        and parts[2].lower() == "retry"
    ):
        from naumi_agent.tools.base import ToolCall

        cancel_receipt_id = parts[3]
        parsed: dict[str, str] = {}
        index = 4
        valid = True
        while index < len(parts):
            ...
            parsed[option] = parts[index + 1]
            index += 2
        cancel_receipt_sha256 = parsed.get("--sha256", "")
        if (
            not valid
            or re.fullmatch(r"hsacr_[0-9a-f]{24}", cancel_receipt_id) is None
            or re.fullmatch(r"[0-9a-f]{64}", cancel_receipt_sha256) is None
        ):
            console.print(f"[yellow]{usage}[/yellow]")
            return
        session = await engine.get_or_create_session()
        run_id = f"manual:{session.id}"
        reason = parsed.get("--reason") or "用户通过命令请求恢复 Sandbox Eval"

        async def publish_retry_progress(
            event: str,
            data: dict[str, object],
        ) -> None:
            if event != "harness_sandbox_eval_progress":
        ...
        result = await engine.execute_tool(
            ToolCall(
                id=f"manual-harness-sandbox-retry-{uuid.uuid4().hex}",
                name="harness_eval_sandbox_retry",
                arguments=json.dumps(
                    {
                        "retry_action_id": f"hsar_{uuid.uuid4().hex[:24]}",
            ...
            on_event=publish_retry_progress,
        )
        console.print(Markdown(result.content))
        return
    ...
    if subcommand == "baseline" and len(parts) == 4 and parts[1].lower() == "compare":
        try:
            ...
            console.print(f"[yellow]Harness Eval 比较参数无效：{exc}[/yellow]")
            return
        console.print(Markdown(render_eval_comparison_run_status(result)))
        return
