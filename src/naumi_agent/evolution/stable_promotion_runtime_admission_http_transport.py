"""Mutually authenticated HTTP transport for stable runtime Admissions."""
...
STABLE_PROMOTION_RUNTIME_ADMISSION_HTTP_PATH = "/v1/stable-promotion/runtime-admissions:receive"
STABLE_PROMOTION_RUNTIME_ADMISSION_SUBMISSION_MEDIA_TYPE = (
    "application/vnd.naumi.stable-promotion-runtime-admission.v1+base64"
)
STABLE_PROMOTION_RUNTIME_ADMISSION_RECEIPT_MEDIA_TYPE = (
    "application/vnd.naumi.stable-promotion-runtime-admission-receipt.v1+base64"
)

_ADMISSION_ID_RE = re.compile(r"^evstablepromadmit_[0-9a-f]{24}$")
_SUBMISSION_ID_RE = re.compile(r"^evstablepromsubmit_[0-9a-f]{24}$")
...
class StablePromotionRuntimeAdmissionHTTPClientPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            validate_tls_file(
                self.server_ca_path,
                label="Runtime Admission Control Plane CA",
                private=False,
            ),
        )
        object.__setattr__(
            ...
            validate_tls_file(
                ...
                label="Runtime Admission installation 证书",
                private=False,
            ),
        )
        object.__setattr__(
            ...
            validate_tls_file(
                ...
                label="Runtime Admission installation 私钥",
                private=True,
            ),
        )
        object.__setattr__(
            ...
            validate_fingerprints(
                ...
                label="Runtime Admission Control Plane 证书 pin",
            ),
        )
        validate_seconds(
            self.connect_timeout_seconds,
            "Runtime Admission 连接 timeout",
            maximum=120,
        )
        validate_seconds(
            self.request_timeout_seconds,
            "Runtime Admission 请求 timeout",
            maximum=600,
        )
        if self.request_timeout_seconds < self.connect_timeout_seconds:
            raise ValueError("Runtime Admission 请求 timeout 不能小于连接 timeout。")
        if not 1 <= self.max_response_bytes <= _MAX_RECEIPT_BYTES:
            raise ValueError("Runtime Admission Receipt 大小上限无效。")


@dataclass(frozen=True, slots=True)
class StablePromotionRuntimeAdmissionHTTPServerPolicy:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            ...
            validate_tls_file(
                ...
                label="Runtime Admission Control Plane 证书",
                private=False,
            ),
        )
        object.__setattr__(
            ...
            validate_tls_file(
                ...
                label="Runtime Admission Control Plane 私钥",
                private=True,
            ),
        )
        object.__setattr__(
        ...
        for member_id, values in raw.items():
            ...
            if _MEMBER_ID_RE.fullmatch(member) is None or member in normalized:
                raise ValueError("Runtime Admission 成员证书表包含无效或重复成员。")
            normalized[member] = validate_fingerprints(
                tuple(values),
                label=f"Runtime Admission 成员 {member} 证书 pin",
            )
        object.__setattr__(
            self,
        ...
        if not 1 <= self.max_request_bytes <= _MAX_SUBMISSION_BYTES:
            raise ValueError("Runtime Admission Submission 大小上限无效。")
        validate_seconds(
            self.tls_handshake_timeout_seconds,
            "Runtime Admission TLS handshake timeout",
            maximum=120,
        )
        validate_seconds(
            self.request_timeout_seconds,
            "Runtime Admission 服务端请求 timeout",
            maximum=600,
        )
        if not 1 <= self.requests_per_minute <= 100_000:
            raise ValueError("Runtime Admission 请求速率上限无效。")
        if not 1 <= self.max_concurrent_requests <= 1024:
            raise ValueError("Runtime Admission 并发请求上限无效。")
...
class MTLSStablePromotionRuntimeAdmissionControlPlaneTransport:
    """Installation-side client with same-connection Control Plane pinning."""

    def __init__(self, policy: StablePromotionRuntimeAdmissionHTTPClientPolicy) -> None:
        self.policy = policy
    ...
    async def submit(self, submission: EvolutionStablePromotionRuntimeAdmissionSubmission):
        item = EvolutionStablePromotionRuntimeAdmissionSubmission.model_validate(submission)
        body = encode_stable_promotion_runtime_admission_submission(item).encode("ascii")
        if len(body) > _MAX_SUBMISSION_BYTES:
            raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                "stable_promotion_admission_http_submission_oversized",
                "Runtime Admission Submission 超过网络传输上限。",
                retryable=False,
            )
        ...
        try:
            async with asyncio.timeout(self.policy.request_timeout_seconds):
                ...
                if headers.get("content-type", "").split(";", 1)[0].strip() != (
                    ...
                    raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                        "stable_promotion_admission_http_receipt_content_type_invalid",
                        "Control Plane 返回了不受支持的 Receipt media type。",
                        retryable=False,
                    )
                try:
                    receipt = decode_stable_promotion_runtime_admission_receipt(
                    ...
                    raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                        "stable_promotion_admission_http_receipt_invalid",
                        "Control Plane 返回的 Runtime Admission Receipt 无效。",
                        retryable=False,
                    ) from exc
                if not stable_promotion_runtime_admission_receipt_matches_submission(receipt, item):
                    raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                        "stable_promotion_admission_http_receipt_mismatch",
                        "Control Plane Receipt 与本次 Submission 不匹配。",
                        retryable=False,
                    )
                return receipt
        except EvolutionStablePromotionRuntimeAdmissionTransportError:
            raise
        except TimeoutError as exc:
            raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                "stable_promotion_admission_http_timeout",
                "Control Plane Runtime Admission 请求超时。",
            ) from exc
        except ssl.SSLCertVerificationError as exc:
            raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                "stable_promotion_admission_http_tls_identity_invalid",
                "Control Plane TLS identity 验证失败。",
                retryable=False,
            ) from exc
        except (ConnectionError, OSError, ssl.SSLError) as exc:
            raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                "stable_promotion_admission_http_unavailable",
                "Control Plane Runtime Admission 端点暂时不可用。",
            ) from exc
        finally:
    ...
    def _verify_peer_pin(self, writer: asyncio.StreamWriter) -> None:
        ...
        if fingerprint not in self.policy.server_certificate_sha256_pins:
            raise EvolutionStablePromotionRuntimeAdmissionTransportError(
                "stable_promotion_admission_http_server_pin_mismatch",
    ...
    def _request(
        ...
        headers = (
            ...
            f"Content-Type: {STABLE_PROMOTION_RUNTIME_ADMISSION_SUBMISSION_MEDIA_TYPE}\r\n"
...
class StablePromotionRuntimeAdmissionHTTPServer:
    """Dedicated Control Plane mTLS endpoint for signed Runtime Admissions."""
    ...
    def __init__(
        ...
        *,
        ...
        service: EvolutionStablePromotionRuntimeAdmissionDeliveryService,
        clock: Callable[[], datetime] = lambda: datetime.now(UTC),
    ) -> None:
        host = str(bind_host or "").strip()
        if not host or len(host) > 255:
            raise ValueError("Runtime Admission 服务 bind host 无效。")
        if isinstance(port, bool) or not isinstance(port, int) or not 0 <= port <= 65535:
            raise ValueError("Runtime Admission 服务 port 无效。")
        if not isinstance(service, EvolutionStablePromotionRuntimeAdmissionDeliveryService):
            raise TypeError("Runtime Admission HTTP 必须使用 7b2a1 Delivery Service。")
        self.bind_host = host
        self.port = port
        self.policy = policy
    ...
    def start(self) -> bool:
        with self._lifecycle_lock:
            ...
            class Handler(_StablePromotionRuntimeAdmissionRequestHandler):
                endpoint = owner
            ...
            self._thread = threading.Thread(
                ...
                name="naumi-stable-runtime-admission-mtls",
                daemon=True,
            )
            self._thread.start()
            return True
    ...
    def stop(self) -> bool:
        with self._lifecycle_lock:
            ...
            self._thread = None
            if thread.is_alive():
                raise RuntimeError("Runtime Admission mTLS 服务未在期限内停止。")
            return True

    def __enter__(self) -> StablePromotionRuntimeAdmissionHTTPServer:
        self.start()
        return self
    ...
    def _receive(self, submission: EvolutionStablePromotionRuntimeAdmissionSubmission):
        async def receive():
            async with asyncio.timeout(self.policy.request_timeout_seconds):
...
class _StablePromotionRuntimeAdmissionRequestHandler(BaseHTTPRequestHandler):
    endpoint: StablePromotionRuntimeAdmissionHTTPServer
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if identity is None or not self._header_envelope_valid():
            return
        if self.path != STABLE_PROMOTION_RUNTIME_ADMISSION_HTTP_PATH:
        ...
        if accept is None:
            return
        if accept.strip() != STABLE_PROMOTION_RUNTIME_ADMISSION_RECEIPT_MEDIA_TYPE:
        ...
        if len(body) != length:
            ...
            return
        try:
            submission = decode_stable_promotion_runtime_admission_submission(body.decode("ascii"))
        except (
            UnicodeDecodeError,
            ValueError,
            EvolutionStablePromotionRuntimeAdmissionDeliveryError,
        ...
        if not (
            admission.admission_id == admission_id and submission.submission_id == submission_id
        ):
        ...
        try:
            ...
            self._error(503, "admission_receive_timeout")
            return
        except EvolutionStablePromotionRuntimeAdmissionDeliveryError as exc:
            self._error(422, exc.code)
            return
        except Exception:
            self._error(500, "admission_receive_failed")
            return
        if not (
            view.remote_admission_delivery_authority
            ...
            return
        encoded = encode_stable_promotion_runtime_admission_receipt(view.receipt).encode("ascii")
...
async def _read_response(
    ...
    *,
    ...
    if not 0 <= length <= max_body_bytes:
        raise EvolutionStablePromotionRuntimeAdmissionTransportError(
            "stable_promotion_admission_http_response_oversized",
    ...
    try:
        ...
        raise EvolutionStablePromotionRuntimeAdmissionTransportError(
            "stable_promotion_admission_http_response_truncated",
...
def _invalid_response(
    ...
    return EvolutionStablePromotionRuntimeAdmissionTransportError(
        "stable_promotion_admission_http_response_invalid",
        message,
        retryable=False,
    )
...
def _status_error(
    ...
    try:
        ...
        pass
    code = (
        f"stable_promotion_admission_http_{remote_code}"
        if remote_code
        else f"stable_promotion_admission_http_status_{status}"
    )
    return EvolutionStablePromotionRuntimeAdmissionTransportError(
        code,
        f"Control Plane 拒绝 Runtime Admission（HTTP {status}，{remote_code or 'unknown'}）。",
        retryable=retryable,
    )
...
def _endpoint(value: str) -> SplitResult:
    ...
    try:
        ...
        raise ValueError("Runtime Admission endpoint URL 无效。") from exc
    if not (
        parsed.scheme == "https"
        and parsed.hostname
        ...
        and parsed.query == ""
        and parsed.fragment == ""
        and parsed.path == STABLE_PROMOTION_RUNTIME_ADMISSION_HTTP_PATH
        and (port is None or 1 <= port <= 65535)
    ):
        raise ValueError("Runtime Admission endpoint 必须是固定路径的 https URL。")
    return parsed
...
def _aware(value: str | datetime) -> datetime:
    ...
    if not (
        ...
        raise ValueError("Runtime Admission HTTP timestamp 必须包含 offset。")
