"""Durable, dynamically revalidated percentage runtime-window assessments."""
...
EVOLUTION_REVALIDATION_PERCENTAGE_OBSERVATION_ASSESSMENT_POLICY = (
    "evolution-revalidation-percentage-observation-assessment-v1"
)
...
class EvolutionRevalidationPercentageObservationWindowView(_StrictModel):
    schema_version: Literal[1] = 1
    policy_version: Literal[
        "evolution-revalidation-percentage-observation-assessment-v1"
    ] = EVOLUTION_REVALIDATION_PERCENTAGE_OBSERVATION_ASSESSMENT_POLICY
    receipt: EvolutionRevalidationPercentageObservationWindow
    current_assessment: EvolutionRevalidationPercentageObservationWindow | None = None
    ...
    def _project(self) -> Self:
        durable_dependencies = bool(
            ...
            and self.deployment_launch_input_authority
        ...
        passing = bool(
            ...
            is EvolutionRevalidationPercentageObservationWindowStatus.PASSING
            and self.current_assessment.percentage_runtime_window_authority
        )
        breached = bool(
            durable_dependencies
            and self.current_assessment is not None
            and self.current_assessment.status
            is EvolutionRevalidationPercentageObservationWindowStatus.BREACHED
            and self.current_assessment.pause_input_authority
            and self.current_assessment.rollback_input_authority
        )
        if not (
            self.percentage_runtime_window_authority is passing
            ...
            raise ValueError("Percentage Observation Window View authority 投影不一致。")
        return self


class EvolutionRevalidationPercentageObservationAssessmentError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class EvolutionRevalidationPercentageObservationWindowStore:
    """Append-only percentage windows backed by exact Exposure and HAR facts."""

    def __init__(
        self,
        db_path: str | Path,
        *,
        exposure_store: EvolutionRevalidationPercentageRuntimeExposureStore,
        ...
        if not isinstance(
            ...
            raise TypeError("Percentage Observation Store 需要 Runtime Exposure Store。")
        if not isinstance(harness_store, HarnessStore):
            raise TypeError("Percentage Observation Store 需要 HarnessStore。")
        self.db_path = Path(db_path).expanduser().resolve()
        if self.db_path != exposure_store.db_path:
            raise ValueError("Percentage Observation 与 Exposure 必须共用证据数据库。")
    ...
    async def get(
        self,
        window_id: str,
    ) -> EvolutionRevalidationPercentageObservationWindow | None:
        ...
        except EvolutionRevalidationPercentageObservationAssessmentError:
            raise
        except (aiosqlite.Error, OSError, TypeError, ValueError) as exc:
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_window_source_unavailable",
                "Percentage Observation Window durable source 当前不可读取。",
            ) from exc
    ...
    async def record(
        ...
        if len(encoded.encode()) > _MAX_WINDOW_BYTES:
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_window_oversized",
                "Percentage Observation Window 超过 8 MiB。",
            )
        await self._require_external_sources(item)
        assignment_id = _window_assignment_id(item)
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                if exposure_row is None or not hmac.compare_digest(
                    ...
                    await db.rollback()
                    raise EvolutionRevalidationPercentageObservationAssessmentError(
                        "percentage_observation_exposure_source_changed",
                        "Percentage Runtime Exposure durable source 已变化或不存在。",
                    )
                existing = await (
                    await db.execute(
                        "SELECT window_json FROM "
                ...
                if existing is not None:
                    ...
                    if restored != item:
                        raise EvolutionRevalidationPercentageObservationAssessmentError(
                            "percentage_observation_window_identity_conflict",
                            "同一 Percentage Window identity 已绑定不同内容。",
                        )
                    return restored
                await db.execute(
                    "INSERT INTO evolution_revalidation_percentage_observation_windows "
                    "(window_id, window_sha256, assignment_id, exposure_id, "
            ...
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_window_store_error",
                "Percentage Observation Window 无法持久化。",
            ) from exc
        return item

    async def _require_external_sources(
        self,
        item: EvolutionRevalidationPercentageObservationWindow,
    ) -> None:
        try:
            exposure = await self.exposure_store.get_by_assignment(
                _window_assignment_id(item)
            )
            ...
            if binding != item.exposure.binding:
                raise EvolutionRevalidationPercentageObservationAssessmentError(
                    "percentage_observation_binding_source_changed",
            ...
            if actual != item.samples:
                raise EvolutionRevalidationPercentageObservationAssessmentError(
                    "percentage_observation_sample_source_changed",
            ...
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_source_unavailable",
                "Percentage Observation Window durable source 当前不可验证。",
            ) from exc


class EvolutionRevalidationPercentageObservationWindowService:
    def __init__(
        self,
        *,
        workspace_root: str | Path,
        exposure_service: EvolutionRevalidationPercentageRuntimeExposureService,
        harness_store: HarnessStore,
        store: EvolutionRevalidationPercentageObservationWindowStore,
        ...
        if not (
            ...
            and isinstance(store, EvolutionRevalidationPercentageObservationWindowStore)
            and store.exposure_store is exposure_service.store
            and store.harness_store is harness_store
        ):
            raise ValueError("Percentage Observation Window Service dependency 不一致。")
        self.workspace_root = Path(workspace_root).expanduser().resolve(strict=True)
        if self.workspace_root != exposure_service.workspace_root:
            raise ValueError("Percentage Observation Window workspace 必须一致。")
    ...
    async def assess(
        ...
        *,
        ...
        assignment = _assignment(assignment_id)
        subject = _subject(subject_id)
        lock = self._locks.setdefault((assignment, subject), asyncio.Lock())
        async with lock:
            exposure_view = await self._inspect_exposure(assignment)
            if not (
                exposure_view.runtime_exposure_fact_authority
                and exposure_view.deployment_launch_input_current
            ):
                raise EvolutionRevalidationPercentageObservationAssessmentError(
                    "percentage_observation_exposure_stale",
                    "Percentage Runtime Exposure 当前不能形成新窗口回执。",
                )
            if exposure_view.receipt.binding.subject_id != subject:
                raise EvolutionRevalidationPercentageObservationAssessmentError(
                    "percentage_observation_subject_mismatch",
                    "subject_id 未绑定 exact Percentage Runtime Exposure。",
                )
            try:
    ...
    async def inspect(
        ...
        *,
        ...
        if receipt is None:
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_window_missing",
                "尚未形成 Percentage Observation Window Receipt。",
            )
    ...
    async def inspect_receipt(
        ...
        *,
        ...
        if receipt is None:
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_window_missing",
                "指定的 Percentage Observation Window Receipt 不存在。",
            )
        return await self._view(receipt, assessed_at=self._now().isoformat())

    async def _inspect_exposure(self, assignment_id: str):
        try:
            return await self.exposure_service.inspect(assignment_id=assignment_id)
        except EvolutionRevalidationPercentageRuntimeExposureError as exc:
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_exposure_missing",
                "缺少可供窗口评估的 Percentage Runtime Exposure。",
            ) from exc
    ...
    async def _view(
        ...
        *,
        ...
        assignment_id = _window_assignment_id(receipt)
        ...
        try:
            exposure_source = await self.store.exposure_store.get_by_assignment(
                assignment_id
            )
        ...
        try:
            exposure_view = await self._inspect_exposure(assignment_id)
        ...
        checks = {
            ...
            "deployment_launch_input_stale": deployment_launch_input_authority,
        ...
        durable_dependencies = bool(
            ...
            and deployment_launch_input_authority
        ...
        return EvolutionRevalidationPercentageObservationWindowView(
            receipt=receipt,
...
async def _read_current_ledger(
    *,
    ...
    if binding is None:
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_binding_missing",
    ...
    if heartbeat is None:
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_heartbeat_missing",
    ...
    if origin_page is None or not origin_page.items:
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_samples_missing",
    ...
    if not (
        ...
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_ledger_binding_mismatch",
    ...
    while after_sequence < heartbeat.sequence:
        ...
        if not page.items or page.items[-1].heartbeat_sequence <= after_sequence:
            raise EvolutionRevalidationPercentageObservationAssessmentError(
                "percentage_observation_ledger_cursor_invalid",
    ...
    if heartbeat_after != heartbeat or not _heartbeat_matches_sample(
        ...
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_ledger_head_changed",
...
def _build_window(*, exposure, samples, assessed_at):
    try:
        return build_percentage_observation_window(
            exposure=exposure,
            samples=samples,
            assessed_at=assessed_at,
        )
    except EvolutionRevalidationPercentageObservationWindowError as exc:
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            exc.code,
            str(exc),
        ) from exc
...
def _validated_window(value) -> EvolutionRevalidationPercentageObservationWindow:
    try:
        return EvolutionRevalidationPercentageObservationWindow.model_validate_json(
            value.model_dump_json()
        )
    except (AttributeError, TypeError, ValueError) as exc:
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_window_invalid",
            "Percentage Observation Window artifact 无效。",
        ) from exc


def _restore_window(value: str) -> EvolutionRevalidationPercentageObservationWindow:
    if len(value.encode()) > _MAX_WINDOW_BYTES:
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_window_source_oversized",
            "Percentage Observation Window durable source 超过 8 MiB。",
        )
    try:
        return EvolutionRevalidationPercentageObservationWindow.model_validate_json(
            value
        ...
        raise EvolutionRevalidationPercentageObservationAssessmentError(
            "percentage_observation_window_source_invalid",
            "Percentage Observation Window durable source 无效。",
        ) from exc


def _window_assignment_id(window) -> str:
    return window.exposure.deployment.preparation.intent.assignment.assignment_id
...
def _aware(value) -> datetime:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError("Percentage Observation timestamp 必须包含 offset。")
...
async def _ensure_schema(db: aiosqlite.Connection) -> None:
    await db.execute(
        """
        CREATE TABLE IF NOT EXISTS evolution_revalidation_percentage_observation_windows (
    ...
    await db.execute(
        ...
        "idx_evolution_revalidation_percentage_observation_windows_latest "
        "ON evolution_revalidation_percentage_observation_windows "
        "(assignment_id, subject_id, assessed_at DESC, window_id DESC)"
    )
    await db.commit()
