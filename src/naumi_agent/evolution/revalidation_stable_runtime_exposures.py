"""Release-bound runtime exposure evidence for stable installations."""
...
EVOLUTION_REVALIDATION_STABLE_RUNTIME_EXPOSURE_POLICY = (
    "evolution-revalidation-stable-runtime-exposure-v1"
)
...
class EvolutionRevalidationStableRuntimeExposureReceipt(_StrictModel):
    ...
    stable_installation_exposure_observed: Literal[True] = True
    ...
    stable_observation_input_authority: Literal[True] = True
    ...
    def _exact(self) -> Self:
        ...
        if not (
            ...
            and deployment.population_membership_enforced
        ...
        if not _startup_pair_matches(
            ...
            raise ValueError("Stable Runtime Exposure startup chain 无效。")
        if not (
            ...
            raise ValueError("Stable Runtime Exposure 时间顺序无效。")
        core = self.model_dump(mode="json", exclude={"exposure_id", "exposure_sha256"})
        digest = _digest(core)
        if not (
            self.exposure_sha256 == digest
            and self.exposure_id == f"evrestableexposure_{digest[:24]}"
        ):
            raise ValueError("Stable Runtime Exposure identity 不一致。")
        return self


class EvolutionRevalidationStableRuntimeExposureView(_StrictModel):
    ...
    stable_observation_input_authority: bool
    user_request_executed: Literal[False] = False
    completed_run_authority: Literal[False] = False
    percentage_rollout_authority: Literal[False] = False
    stable_rollout_authority: Literal[False] = False
    ...
    def _project(self) -> Self:
        fact = bool(
            self.receipt.stable_installation_exposure_observed
        ...
        if not (
            ...
            and self.stable_observation_input_authority is current
            and self.invalidation_reasons
            == tuple(sorted(set(self.invalidation_reasons)))
        ):
            raise ValueError("Stable Runtime Exposure View authority 投影不一致。")
        return self


class EvolutionRevalidationStableRuntimeExposureError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class EvolutionRevalidationStableRuntimeExposureStore:
    def __init__(
        self,
        db_path: str | Path,
        *,
        deployment_store: EvolutionRevalidationStableDeploymentStore,
        harness_store: HarnessStore,
    ) -> None:
        if not isinstance(deployment_store, EvolutionRevalidationStableDeploymentStore):
            raise TypeError("Stable Runtime Exposure Store 需要 Deployment Store。")
        if not isinstance(harness_store, HarnessStore):
            raise TypeError("Stable Runtime Exposure Store 需要 HarnessStore。")
        self.db_path = Path(db_path).expanduser().resolve()
        if self.db_path != deployment_store.db_path:
            raise ValueError("Stable Runtime Exposure 必须共用 Evolution evidence DB。")
    ...
    async def get_by_intent(
        self,
        intent_id: str,
    ) -> EvolutionRevalidationStableRuntimeExposureReceipt | None:
        ...
        try:
            ...
            raise EvolutionRevalidationStableRuntimeExposureError(
                "stable_runtime_exposure_source_invalid",
                "Stable Runtime Exposure durable source 无效。",
            ) from exc
    ...
    async def record(
        ...
        if len(encoded.encode()) > _MAX_ARTIFACT_BYTES:
            raise EvolutionRevalidationStableRuntimeExposureError(
                "stable_runtime_exposure_oversized",
                "Stable Runtime Exposure Receipt 超过 16 MiB。",
            )
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                intent_id = item.deployment.preparation.intent.intent_id
                existing = await (
                    await db.execute(
                        "SELECT exposure_json FROM "
                        "evolution_revalidation_stable_runtime_exposures "
                        "WHERE deployment_receipt_id = ? OR intent_id = ?",
                        (item.deployment.receipt_id, intent_id),
                    )
                ).fetchone()
                if existing is not None:
                    ...
                    if restored != item:
                        raise EvolutionRevalidationStableRuntimeExposureError(
                            "stable_runtime_exposure_conflict",
                            "同一 Stable Deployment 已绑定不同 Runtime Exposure。",
                        )
                    return restored
                await db.execute(
                    "INSERT INTO evolution_revalidation_stable_runtime_exposures "
                    "(exposure_id, exposure_sha256, deployment_receipt_id, "
                    "intent_id, binding_id, subject_id, exposure_json, exposed_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        item.exposure_id,
            ...
            raise EvolutionRevalidationStableRuntimeExposureError(
                "stable_runtime_exposure_store_error",
                "Stable Runtime Exposure Receipt 无法持久化。",
            ) from exc
        return item


class EvolutionRevalidationStableRuntimeExposureService:
    def __init__(
        self,
        *,
        workspace_root: str | Path,
        deployment_service: EvolutionRevalidationStableDeploymentService,
        harness_store: HarnessStore,
        store: EvolutionRevalidationStableRuntimeExposureStore,
        clock: Callable[[], datetime] | None = None,
    ) -> None:
        if not (
            isinstance(deployment_service, EvolutionRevalidationStableDeploymentService)
            and isinstance(harness_store, HarnessStore)
            and isinstance(store, EvolutionRevalidationStableRuntimeExposureStore)
            and store.deployment_store is deployment_service.store
            and store.harness_store is harness_store
        ):
            raise ValueError("Stable Runtime Exposure Service dependency 不一致。")
        self.workspace_root = Path(workspace_root).expanduser().resolve(strict=True)
        if self.workspace_root != deployment_service.workspace_root:
            raise ValueError("Stable Runtime Exposure workspace 必须一致。")
    ...
    async def record(self, *, intent_id: str, subject_id: str):
        _require_intent_id(intent_id)
        ...
        async with lock:
            ...
            if existing is not None:
                if existing.binding.subject_id != subject_id:
                    raise EvolutionRevalidationStableRuntimeExposureError(
                        "stable_runtime_exposure_already_recorded",
                        "该 Stable Deployment 已绑定另一条 Runtime Exposure。",
                    )
            ...
            except EvolutionRevalidationStableDeploymentError as exc:
                raise EvolutionRevalidationStableRuntimeExposureError(
                    "stable_runtime_exposure_deployment_unavailable",
                    "缺少 current Stable Deployment authority。",
                ) from exc
            if not deployment_view.stable_runtime_launch_input_authority:
                raise EvolutionRevalidationStableRuntimeExposureError(
                    "stable_runtime_exposure_deployment_denied",
                    "Stable Deployment 当前不允许记录 runtime exposure。",
                )
            ...
            refreshed = await self.deployment_service.inspect(intent_id=intent_id)
            if not (
                refreshed.receipt == deployment
                and refreshed.stable_runtime_launch_input_authority
            ):
                raise EvolutionRevalidationStableRuntimeExposureError(
                    "stable_runtime_exposure_context_changed",
                    "记录期间 Stable Deployment authority 已变化。",
                )
    ...
    async def inspect(self, *, intent_id: str):
        ...
        if receipt is None:
            raise EvolutionRevalidationStableRuntimeExposureError(
                "stable_runtime_exposure_missing",
                "指定 Stable Intent 尚无 Runtime Exposure Receipt。",
            )
    ...
    async def _view(self, receipt):
        ...
        try:
            stored = await self.store.get_by_intent(intent_id)
        ...
        try:
            ...
            deployment_launch_input_current = bool(
                deployment_view.receipt == receipt.deployment
                and deployment_view.stable_runtime_launch_input_authority
            )
        ...
        return EvolutionRevalidationStableRuntimeExposureView(
            ...
            stable_observation_input_authority=current,
        )
...
def _build_receipt(*, workspace_root, deployment, binding, startup, ready):
    core = {
        "schema_version": 1,
        "policy_version": EVOLUTION_REVALIDATION_STABLE_RUNTIME_EXPOSURE_POLICY,
        ...
        "stable_observation_input_authority": True,
    ...
    digest = _digest(core)
    return EvolutionRevalidationStableRuntimeExposureReceipt.model_validate(
        {
            **core,
            "exposure_id": f"evrestableexposure_{digest[:24]}",
...
def _identity_matches_deployment(identity, deployment) -> bool:
    ...
    return bool(
        ...
        and identity.boot_receipt_id == deployment.preparation.boot_receipt.receipt_id
...
async def _require_harness_sources(harness_store, receipt) -> None:
    ...
    if not (
        ...
        raise EvolutionRevalidationStableRuntimeExposureError(
            "stable_runtime_exposure_harness_changed",
...
async def _require_deployment_source(db, deployment) -> None:
    row = await (
        await db.execute(
            "SELECT receipt_json FROM evolution_revalidation_stable_deployments "
            "WHERE receipt_id = ?",
    ...
    try:
        durable = (
            None
            if row is None
            else EvolutionRevalidationStableDeploymentReceipt.model_validate_json(
                row["receipt_json"]
            )
        )
    except (TypeError, ValueError) as exc:
        raise EvolutionRevalidationStableRuntimeExposureError(
            "stable_runtime_exposure_deployment_invalid",
            "Durable Stable Deployment source 无效。",
        ) from exc
    if durable != deployment:
        raise EvolutionRevalidationStableRuntimeExposureError(
            "stable_runtime_exposure_deployment_changed",
            "Durable Stable Deployment source 已变化。",
        )


def _validated(value) -> EvolutionRevalidationStableRuntimeExposureReceipt:
    try:
        return EvolutionRevalidationStableRuntimeExposureReceipt.model_validate_json(
            value.model_dump_json()
        )
    except (AttributeError, TypeError, ValueError) as exc:
        raise EvolutionRevalidationStableRuntimeExposureError(
            "stable_runtime_exposure_invalid",
            "Stable Runtime Exposure Receipt artifact 无效。",
        ) from exc


def _restore(value: str) -> EvolutionRevalidationStableRuntimeExposureReceipt:
    if len(value.encode()) > _MAX_ARTIFACT_BYTES:
        raise ValueError("Stable Runtime Exposure Receipt 超过 16 MiB。")
    return EvolutionRevalidationStableRuntimeExposureReceipt.model_validate_json(value)
...
def _require_intent_id(value) -> None:
    if not isinstance(value, str) or re.fullmatch(
        ...
        raise EvolutionRevalidationStableRuntimeExposureError(
            "stable_runtime_exposure_intent_invalid",
...
def _require_subject_id(value) -> None:
    if not isinstance(value, str) or _SUBJECT_RE.fullmatch(value) is None:
        raise EvolutionRevalidationStableRuntimeExposureError(
            "stable_runtime_exposure_subject_invalid",
...
async def _ensure_schema(db) -> None:
    await db.execute(
        "CREATE TABLE IF NOT EXISTS evolution_revalidation_stable_runtime_exposures ("
        "exposure_id TEXT PRIMARY KEY, exposure_sha256 TEXT NOT NULL UNIQUE, "
        "deployment_receipt_id TEXT NOT NULL UNIQUE, intent_id TEXT NOT NULL UNIQUE, "
...
def _aware(value) -> datetime:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError("Stable Runtime Exposure timestamp 必须包含 offset。")
