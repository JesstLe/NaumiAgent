"""Continuous platform-scoped Fresh Adversarial RED/GREEN cohorts."""

from __future__ import annotations

import hashlib
import hmac
import json
import re
from collections import Counter
from collections.abc import Callable
from datetime import UTC, datetime
from pathlib import Path
from typing import Literal, Self

import aiosqlite
from pydantic import BaseModel, ConfigDict, Field, field_validator, model_validator

from naumi_agent.daemons.permission_decisions import PermissionDecisionReceiptStore
from naumi_agent.daemons.run_delegation_grants import RunDelegationGrantAuthority
from naumi_agent.evolution.interventional_cohort_kernel import (
    EvolutionInterventionalCohortKernel,
    EvolutionInterventionalCohortKernelError,
)
from naumi_agent.evolution.revalidation_adversarial_samples import (
    EVOLUTION_REVALIDATION_ADVERSARIAL_RUNNER,
    EvolutionRevalidationAdversarialSampleError,
    EvolutionRevalidationAdversarialSampleExecutor,
    EvolutionRevalidationAdversarialSampleStore,
    adversarial_batch_id,
)
from naumi_agent.evolution.revalidation_runtime_contracts import (
    EvolutionRevalidationRuntimeContractService,
)
from naumi_agent.harness.eval_models import EvalCaseStatus
from naumi_agent.harness.sandbox_batch import HarnessSandboxBatchAdmission
from naumi_agent.harness.store import HarnessStore

EVOLUTION_REVALIDATION_ADVERSARIAL_COHORT_POLICY = (
    "evolution-revalidation-adversarial-cohort-v1"
)
_SHA256_RE = r"^[0-9a-f]{64}$"
type Platform = Literal["linux", "macos", "windows"]


class _StrictModel(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
        frozen=True,
        allow_inf_nan=False,
        hide_input_in_errors=True,
    )


class EvolutionRevalidationAdversarialCheckSummary(_StrictModel):
    check_id: str = Field(pattern=r"^[a-z][a-z0-9_-]{0,63}$")
    metric_name: str = Field(pattern=r"^adversarial\.[a-z][a-z0-9_-]{0,63}\.exit_zero$")
    red_passed: int = Field(ge=0, le=100)
    red_failed: int = Field(ge=0, le=100)
    red_evaluation_errors: int = Field(ge=0, le=100)
    green_passed: int = Field(ge=0, le=100)
    green_failed: int = Field(ge=0, le=100)
    green_evaluation_errors: int = Field(ge=0, le=100)
    red_values: tuple[float | None, ...] = Field(min_length=5, max_length=100)
    green_values: tuple[float | None, ...] = Field(min_length=5, max_length=100)


class EvolutionRevalidationAdversarialCohortReceipt(_StrictModel):
    schema_version: Literal[1] = 1
    policy_version: Literal[
        "evolution-revalidation-adversarial-cohort-v1"
    ] = EVOLUTION_REVALIDATION_ADVERSARIAL_COHORT_POLICY
    receipt_id: str = Field(pattern=r"^evrevaladvcohort_[0-9a-f]{24}$")
    receipt_sha256: str = Field(pattern=_SHA256_RE)
    workspace_root: str = Field(min_length=1, max_length=4_096)
    contract_id: str = Field(pattern=r"^evrevalruntime_[0-9a-f]{24}$")
    contract_sha256: str = Field(pattern=_SHA256_RE)
    validation_plan_id: str = Field(pattern=r"^evrevalvplan_[0-9a-f]{24}$")
    validation_plan_sha256: str = Field(pattern=_SHA256_RE)
    source_snapshot_id: str = Field(pattern=r"^evrevalsrc_[0-9a-f]{24}$")
    source_snapshot_sha256: str = Field(pattern=_SHA256_RE)
    platform: Platform
    suite_id: str = Field(pattern=r"^[a-z][a-z0-9_-]{0,63}$")
    red_batch_id: str = Field(min_length=1, max_length=128)
    green_batch_id: str = Field(min_length=1, max_length=128)
    requested_samples: int = Field(ge=5, le=100)
    persisted_samples: int = Field(ge=5, le=100)
    sample_seeds: tuple[int, ...] = Field(min_length=5, max_length=100)
    sample_receipt_sha256: tuple[str, ...] = Field(min_length=5, max_length=100)
    red_result_sha256: tuple[str, ...] = Field(min_length=5, max_length=100)
    green_result_sha256: tuple[str, ...] = Field(min_length=5, max_length=100)
    cohort_run_grant_sha256: tuple[str, ...] = Field(min_length=1, max_length=100)
    platform_sha256: str = Field(pattern=_SHA256_RE)
    checks: tuple[EvolutionRevalidationAdversarialCheckSummary, ...] = Field(
        min_length=1,
        max_length=80,
    )
    continuous_sample_indexes_verified: Literal[True] = True
    identical_environment_verified: Literal[True] = True
    profile_trust_revalidated: Literal[True] = True
    source_current_after_execution: Literal[True] = True
    platform_scoped_run_grant_used: Literal[True] = True
    exact_platform_worker_used: Literal[True] = True
    arc04_worker_used: Literal[True] = True
    project_code_executed: Literal[True] = True
    cohort_complete: Literal[True] = True
    matrix_complete: Literal[False] = False
    comparison_authority: Literal[False] = False
    promotion_authority: Literal[False] = False
    completed_at: str = Field(min_length=1, max_length=100)

    @field_validator("cohort_run_grant_sha256")
    @classmethod
    def _grants(cls, values):
        if values != tuple(sorted(set(values))) or any(
            re.fullmatch(_SHA256_RE, item) is None for item in values
        ):
            raise ValueError("Fresh Adversarial cohort Run Grant 集合无效。")
        return values

    @model_validator(mode="after")
    def _exact(self) -> Self:
        count = self.requested_samples
        if not (
            self.persisted_samples
            == count
            == len(self.sample_seeds)
            == len(self.sample_receipt_sha256)
            == len(self.red_result_sha256)
            == len(self.green_result_sha256)
        ):
            raise ValueError("Fresh Adversarial cohort 样本不完整。")
        for item in self.checks:
            if not (
                len(item.red_values) == len(item.green_values) == count
                and item.red_passed + item.red_failed + item.red_evaluation_errors == count
                and item.green_passed + item.green_failed + item.green_evaluation_errors
                == count
                and item.metric_name == f"adversarial.{item.check_id}.exit_zero"
            ):
                raise ValueError("Fresh Adversarial cohort check summary 不完整。")
        if datetime.fromisoformat(self.completed_at).utcoffset() is None:
            raise ValueError("Fresh Adversarial cohort completed_at 必须包含 offset。")
        digest = _sha256_payload(
            self.model_dump(mode="json", exclude={"receipt_id", "receipt_sha256"})
        )
        if not hmac.compare_digest(self.receipt_sha256, digest):
            raise ValueError("Fresh Adversarial cohort receipt digest 不一致。")
        if self.receipt_id != f"evrevaladvcohort_{digest[:24]}":
            raise ValueError("Fresh Adversarial cohort receipt identity 不一致。")
        return self


class EvolutionRevalidationAdversarialCohortError(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        super().__init__(message)
        self.code = code


class EvolutionRevalidationAdversarialCohortStore:
    def __init__(self, db_path: str | Path) -> None:
        self._db_path = Path(db_path).expanduser().resolve()

    async def get(self, contract_id, platform):
        if not self._db_path.exists():
            return None
        async with aiosqlite.connect(self._db_path) as db:
            db.row_factory = aiosqlite.Row
            await _ensure_schema(db)
            row = await (
                await db.execute(
                    "SELECT receipt_json FROM evolution_revalidation_adversarial_cohorts "
                    "WHERE contract_id = ? AND platform = ?",
                    (contract_id, platform),
                )
            ).fetchone()
        return None if row is None else (
            EvolutionRevalidationAdversarialCohortReceipt.model_validate_json(
                row["receipt_json"]
            )
        )

    async def record(self, receipt):
        item = EvolutionRevalidationAdversarialCohortReceipt.model_validate_json(
            receipt.model_dump_json()
        )
        self._db_path.parent.mkdir(parents=True, exist_ok=True)
        async with aiosqlite.connect(self._db_path) as db:
            db.row_factory = aiosqlite.Row
            await _ensure_schema(db)
            await db.execute("BEGIN IMMEDIATE")
            dependency = await (
                await db.execute(
                    "SELECT contract_sha256 FROM evolution_revalidation_runtime_contracts "
                    "WHERE contract_id = ?",
                    (item.contract_id,),
                )
            ).fetchone()
            if dependency is None or dependency["contract_sha256"] != item.contract_sha256:
                await db.rollback()
                raise EvolutionRevalidationAdversarialCohortError(
                    "fresh_adversarial_cohort_contract_mismatch",
                    "持久化 Runtime Contract 不存在或 digest 不一致。",
                )
            existing = await (
                await db.execute(
                    "SELECT receipt_json FROM evolution_revalidation_adversarial_cohorts "
                    "WHERE contract_id = ? AND platform = ?",
                    (item.contract_id, item.platform),
                )
            ).fetchone()
            if existing is not None:
                restored = EvolutionRevalidationAdversarialCohortReceipt.model_validate_json(
                    existing["receipt_json"]
                )
                await db.rollback()
                if restored != item:
                    raise EvolutionRevalidationAdversarialCohortError(
                        "fresh_adversarial_cohort_conflict",
                        "同一平台已绑定不同 Fresh Adversarial cohort。",
                    )
                return restored
            await db.execute(
                "INSERT INTO evolution_revalidation_adversarial_cohorts "
                "(receipt_id, receipt_sha256, contract_id, platform, receipt_json, "
                "completed_at) VALUES (?, ?, ?, ?, ?, ?)",
                (
                    item.receipt_id,
                    item.receipt_sha256,
                    item.contract_id,
                    item.platform,
                    item.model_dump_json(),
                    item.completed_at,
                ),
            )
            await db.commit()
        return item


class EvolutionRevalidationAdversarialCohortExecutor:
    def __init__(
        self,
        *,
        workspace_root: str | Path,
        harness_store: HarnessStore,
        sample_store: EvolutionRevalidationAdversarialSampleStore,
        receipt_store: EvolutionRevalidationAdversarialCohortStore,
        permission_store: PermissionDecisionReceiptStore,
        run_grant_authority: RunDelegationGrantAuthority,
        sample_executor: EvolutionRevalidationAdversarialSampleExecutor,
        contract_service: EvolutionRevalidationRuntimeContractService,
        now: Callable[[], str] | None = None,
        token: Callable[[], str] | None = None,
        batch_admission: HarnessSandboxBatchAdmission | None = None,
    ) -> None:
        self.workspace_root = Path(workspace_root).expanduser().resolve(strict=True)
        self.harness_store = harness_store
        self.sample_store = sample_store
        self.receipt_store = receipt_store
        self.sample_executor = sample_executor
        self.contract_service = contract_service
        self.now = now or (lambda: datetime.now(UTC).isoformat())
        self.kernel = EvolutionInterventionalCohortKernel(
            workspace_root=self.workspace_root,
            store=harness_store,
            permission_store=permission_store,
            run_grant_authority=run_grant_authority,
            now=self.now,
            token=token,
            admission=batch_admission,
        )

    async def execute(self, *, contract_id, platform: Platform, parent_receipt_id):
        view = await self.contract_service.inspect(
            workspace_root=self.workspace_root,
            contract_id=contract_id,
        )
        if not view.execution_eligible or platform not in view.contract.required_platforms:
            raise EvolutionRevalidationAdversarialCohortError(
                "fresh_adversarial_cohort_not_ready",
                "Fresh Runtime Contract 或 platform lane 不可执行。",
            )
        contract = view.contract

        async def load_records():
            return await self._records(contract, platform, "red")

        async def validate_prefix(records):
            return [
                await self.sample_executor.execute(
                    contract_id=contract.contract_id,
                    platform=platform,
                    parent_receipt_id="existing-adversarial-cohort-sample",
                    sample_index=item.sample_index,
                    run_scope="cohort",
                )
                for item in records
            ]

        def validate_evidence(records):
            _validate_run_evidence(records, contract)

        async def execute_sample(index, authority):
            return await self.sample_executor.execute(
                contract_id=contract.contract_id,
                platform=platform,
                parent_receipt_id=parent_receipt_id,
                sample_index=index,
                run_scope="cohort",
                run_authority=authority,
            )

        try:
            await self.kernel.execute(
                phase="adversarial",
                authority_key=_authority_key(contract, platform),
                parent_receipt_id=parent_receipt_id,
                requested_samples=contract.requested_samples,
                max_total_duration_seconds=contract.max_total_duration_seconds,
                load_records=load_records,
                validate_existing_prefix=validate_prefix,
                validate_run_evidence=validate_evidence,
                execute_sample=execute_sample,
                build_receipt=lambda _records, receipts: receipts[-1],
            )
        except (
            EvolutionInterventionalCohortKernelError,
            EvolutionRevalidationAdversarialSampleError,
        ) as exc:
            raise EvolutionRevalidationAdversarialCohortError(
                getattr(exc, "code", "fresh_adversarial_cohort_failed"),
                str(exc),
            ) from exc
        red = await self._records(contract, platform, "red")
        green = await self._records(contract, platform, "green")
        receipts = tuple([
            await self.sample_store.get(contract.contract_id, platform, index, "cohort")
            for index in range(contract.requested_samples)
        ])
        if any(item is None for item in receipts):
            raise EvolutionRevalidationAdversarialCohortError(
                "fresh_adversarial_cohort_receipts_incomplete",
                "Fresh Adversarial cohort sample receipts 不完整。",
            )
        return await self.receipt_store.record(
            _build_receipt(contract, platform, red, green, receipts)
        )

    async def _records(self, contract, platform, phase):
        return await self.harness_store.list_eval_results(
            self.workspace_root,
            adversarial_batch_id(contract, platform, phase, "cohort"),
            contract.suite_id,
            limit=contract.requested_samples + 1,
        )


def _build_receipt(contract, platform, red, green, receipts):
    count = contract.requested_samples
    indexes = tuple(range(count))
    if not (
        len(red) == len(green) == len(receipts) == count
        and tuple(item.sample_index for item in red) == indexes
        and tuple(item.sample_index for item in green) == indexes
        and tuple(item.sample_index for item in receipts) == indexes
        and tuple(item.red_result_sha256 for item in receipts)
        == tuple(item.result_sha256 for item in red)
        and tuple(item.green_result_sha256 for item in receipts)
        == tuple(item.result_sha256 for item in green)
        and len({item.platform_sha256 for item in receipts}) == 1
        and all(item.check_ids == receipts[0].check_ids for item in receipts)
    ):
        raise EvolutionRevalidationAdversarialCohortError(
            "fresh_adversarial_cohort_prefix_incomplete",
            "Fresh Adversarial RED/GREEN 前缀不完整。",
        )
    checks = []
    for check_id in receipts[0].check_ids:
        red_statuses = _statuses(red, check_id)
        green_statuses = _statuses(green, check_id)
        checks.append(EvolutionRevalidationAdversarialCheckSummary(
            check_id=check_id,
            metric_name=f"adversarial.{check_id}.exit_zero",
            red_passed=red_statuses[EvalCaseStatus.PASSED.value],
            red_failed=red_statuses[EvalCaseStatus.IMPLEMENTATION_FAILURE.value],
            red_evaluation_errors=red_statuses[EvalCaseStatus.EVALUATION_ERROR.value],
            green_passed=green_statuses[EvalCaseStatus.PASSED.value],
            green_failed=green_statuses[EvalCaseStatus.IMPLEMENTATION_FAILURE.value],
            green_evaluation_errors=green_statuses[EvalCaseStatus.EVALUATION_ERROR.value],
            red_values=_values(red, check_id),
            green_values=_values(green, check_id),
        ))
    grants = tuple(sorted({item.red_run_grant_sha256 for item in receipts}))
    if any(
        item.run_scope != "cohort"
        or item.platform != platform
        or item.red_run_grant_sha256 != item.green_run_grant_sha256
        for item in receipts
    ):
        raise EvolutionRevalidationAdversarialCohortError(
            "fresh_adversarial_cohort_sample_mismatch",
            "Fresh Adversarial sample scope/platform/Run Grant 不一致。",
        )
    payload = {
        "schema_version": 1,
        "policy_version": EVOLUTION_REVALIDATION_ADVERSARIAL_COHORT_POLICY,
        "workspace_root": contract.workspace_root,
        "contract_id": contract.contract_id,
        "contract_sha256": contract.contract_sha256,
        "validation_plan_id": contract.validation_plan_id,
        "validation_plan_sha256": contract.validation_plan_sha256,
        "source_snapshot_id": contract.source_snapshot_id,
        "source_snapshot_sha256": contract.source_snapshot_sha256,
        "platform": platform,
        "suite_id": contract.suite_id,
        "red_batch_id": red[0].batch_id,
        "green_batch_id": green[0].batch_id,
        "requested_samples": count,
        "persisted_samples": count,
        "sample_seeds": [item.sample_seed for item in receipts],
        "sample_receipt_sha256": [item.receipt_sha256 for item in receipts],
        "red_result_sha256": [item.result_sha256 for item in red],
        "green_result_sha256": [item.result_sha256 for item in green],
        "cohort_run_grant_sha256": list(grants),
        "platform_sha256": receipts[0].platform_sha256,
        "checks": [item.model_dump(mode="json") for item in checks],
        "continuous_sample_indexes_verified": True,
        "identical_environment_verified": True,
        "profile_trust_revalidated": True,
        "source_current_after_execution": True,
        "platform_scoped_run_grant_used": True,
        "exact_platform_worker_used": True,
        "arc04_worker_used": True,
        "project_code_executed": True,
        "cohort_complete": True,
        "matrix_complete": False,
        "comparison_authority": False,
        "promotion_authority": False,
        "completed_at": max(item.completed_at for item in receipts),
    }
    digest = _sha256_payload(payload)
    return EvolutionRevalidationAdversarialCohortReceipt.model_validate({
        **payload,
        "receipt_id": f"evrevaladvcohort_{digest[:24]}",
        "receipt_sha256": digest,
    })


def _validate_run_evidence(records, contract):
    if len(records) > contract.requested_samples:
        raise EvolutionRevalidationAdversarialCohortError(
            "fresh_adversarial_cohort_overflow", "Adversarial cohort 超出样本预算。"
        )
    expected = tuple(item.check_id for item in contract.probe_checks)
    for record in records:
        cases = record.result.cases
        grants = {
            match.group(1)
            for item in cases
            if "run_scope=cohort" in item.message
            and (match := re.search(r"run_grant_sha256=([0-9a-f]{64})(?:\s|$)", item.message))
        }
        if (
            tuple(item.case_id for item in cases) != expected
            or any(item.runner != EVOLUTION_REVALIDATION_ADVERSARIAL_RUNNER for item in cases)
            or len(grants) != 1
        ):
            raise EvolutionRevalidationAdversarialCohortError(
                "fresh_adversarial_cohort_run_evidence_incomplete",
                "Adversarial cohort 缺少 platform-scoped Run Grant evidence。",
            )


def _values(records, check_id):
    return tuple(
        None if not case.metric_observations else case.metric_observations[0].value
        for record in records
        for case in record.result.cases
        if case.case_id == check_id
    )


def _statuses(records, check_id):
    return Counter(
        case.status.value
        for record in records
        for case in record.result.cases
        if case.case_id == check_id
    )


def _authority_key(contract, platform):
    return hashlib.sha256(
        f"{contract.contract_sha256}:{platform}:adversarial-cohort".encode()
    ).hexdigest()


async def _ensure_schema(db):
    await db.execute(
        "CREATE TABLE IF NOT EXISTS evolution_revalidation_adversarial_cohorts ("
        "receipt_id TEXT PRIMARY KEY, receipt_sha256 TEXT NOT NULL UNIQUE, "
        "contract_id TEXT NOT NULL, "
        "platform TEXT NOT NULL CHECK (platform IN ('linux', 'macos', 'windows')), "
        "receipt_json TEXT NOT NULL, "
        "completed_at TEXT NOT NULL, UNIQUE (contract_id, platform))"
    )
    await db.commit()


def _sha256_payload(payload):
    return hashlib.sha256(json.dumps(
        payload, ensure_ascii=False, allow_nan=False, sort_keys=True,
        separators=(",", ":"),
    ).encode()).hexdigest()


__all__ = [
    "EVOLUTION_REVALIDATION_ADVERSARIAL_COHORT_POLICY",
    "EvolutionRevalidationAdversarialCheckSummary",
    "EvolutionRevalidationAdversarialCohortError",
    "EvolutionRevalidationAdversarialCohortExecutor",
    "EvolutionRevalidationAdversarialCohortReceipt",
    "EvolutionRevalidationAdversarialCohortStore",
]
