...
DEFAULT_OWNERSHIP_RULES: tuple[OwnershipRule, ...] = (
    ...
    _package_rule(DomainOwner.RUNTIME, "pi_engine", "pi coding agent 运行时桥接"),
    _module_rule(DomainOwner.RUNTIME, "parallel_sessions", "独立终端会话进程启动"),
    _module_rule(DomainOwner.RUNTIME, "deploy", "部署入口"),
    _module_rule(DomainOwner.RUNTIME, "packaging_entry", "打包启动入口"),
    _module_rule(DomainOwner.RUNTIME, "release_launcher_entry", "发布启动入口"),
    ...
    _module_rule(DomainOwner.TOOLS, "analysis_scan", "确定性静态分析 CLI"),
