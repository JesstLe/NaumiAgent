...
### Fixed
- **CLI Evolve Result Wrapper Unwrap** — `/evolve` 现在会在顶层缺少有效修改字段时解包 LLM 输出中的 `result` 对象，避免常见最终结果包装中断自我修改流程。
...
### Fixed
- **CLI Evolve Proposal Wrapper Unwrap** — `/evolve` 现在会在顶层缺少有效修改字段时解包 LLM 输出中的 `proposal` 对象，避免常见外层方案包装中断自我修改流程。
...
### Fixed
- **CLI Evolve Code Content Alias** — `/evolve` 现在会接受 LLM 修改方案中的 `code` 作为 `new_content` 别名，避免常见代码字段命名差异中断自我修改流程。

## [0.1.192] - 2026-06-13

### Fixed
- **CLI Evolve Updated Content Alias** — `/evolve` 现在会接受 LLM 修改方案中的 `updated_content` 作为 `new_content` 别名，避免常见修改后内容字段命名差异中断自我修改流程。
...
### Fixed
- **CLI Evolve New File Content Alias** — `/evolve` 现在会接受 LLM 修改方案中的 `new_file_content` 作为 `new_content` 别名，避免完整文件内容字段命名差异中断自我修改流程。

## [0.1.189] - 2026-06-13

### Fixed
- **CLI Evolve Content Field Alias** — `/evolve` 现在会接受 LLM 修改方案中的 `content` 作为 `new_content` 别名，避免常见内容字段命名差异中断自我修改流程。

## [0.1.188] - 2026-06-13

### Fixed
- **CLI Evolve Target Field Alias** — `/evolve` 现在会接受 LLM 修改方案中的 `file_path` 作为 `target_file` 别名，避免常见字段命名差异中断自我修改流程。
...
### Fixed
- **CLI Evolve Src Module Target Normalization** — `/evolve` 现在会把 LLM 输出的 `src.naumi_agent.tools.analysis` 这类模块名归一化为 `tools/analysis.py`，避免 Python 源码布局前缀导致合法目标被拒绝。
