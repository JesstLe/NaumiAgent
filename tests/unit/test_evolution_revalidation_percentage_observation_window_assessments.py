...
def _assessment_service(data, *, exposure_service, exposure_store):
    ...
    service = EvolutionRevalidationPercentageObservationWindowService(
...
async def test_durable_percentage_window_revalidates_failure_and_pointer_drift(
    tmp_path: Path,
) -> None:
    subject_id = "percentage-assessment-primary"
    ...
    assert view.receipt.status is (
        EvolutionRevalidationPercentageObservationWindowStatus.PASSING
    )
    ...
    assert view.percentage_runtime_window_authority
    assert not view.completed_run_evidence_authority
    assert not view.cohort_observation_authority
    ...
    assert unchanged.percentage_runtime_window_authority
    ...
    assert not stale.percentage_runtime_window_authority
    ...
    assert not breached.percentage_runtime_window_authority
    ...
    assert not drifted.percentage_runtime_window_authority
    assert not drifted.pause_input_authority
    assert "deployment_launch_input_stale" in drifted.invalidation_reasons

    with sqlite3.connect(store_a.db_path) as db:
        db.execute(
            "UPDATE evolution_revalidation_percentage_observation_windows "
    ...
    assert tampered.value.code == "percentage_observation_window_source_invalid"


@pytest.mark.asyncio
@pytest.mark.skipif(os.name == "nt", reason="真实 release fixture 使用 POSIX executable")
async def test_percentage_assessment_reads_verified_bounded_suffix(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    subject_id = "percentage-assessment-suffix"
    data, lifecycle, exposure = await _started_exposure(
        tmp_path,
        subject_id=subject_id,
    )
