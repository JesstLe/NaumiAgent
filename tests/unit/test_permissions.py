...
class TestPermissionChecker:
    ...
    def test_stable_rollout_authorization_is_medium_without_confirmation(
        self,
        mode: PermissionMode,
    ) -> None:
        result = PermissionChecker(mode).check(
            ...
            {
                "action": "inspect",
                "authorization_id": "evstablerolloutauth_" + "a" * 24,
            },
        )
        assert result.allowed
        assert not result.requires_confirmation
        assert result.risk_level is (
            PermissionRiskLevel.LOW
            if mode is PermissionMode.BYPASS
            else PermissionRiskLevel.MEDIUM
        )
        if mode is not PermissionMode.BYPASS:
            assert result.tool_family == "evolution_release_authorization"
    ...
    def test_stable_remote_readiness_is_direct_and_bypass_has_no_confirmation(
        self,
    ) -> None:
    ...
    def test_rollout_control_key_is_explicit_and_bypass_never_prompts(self) -> None:
        moderate = PermissionChecker(PermissionMode.MODERATE).check(
    ...
    def test_remote_finalization_authorization_never_prompts_in_bypass(self) -> None:
        moderate = PermissionChecker(PermissionMode.MODERATE).check(
    ...
    def test_remote_finalization_execution_never_prompts_in_bypass(self) -> None:
        moderate = PermissionChecker(PermissionMode.MODERATE).check(
    ...
    def test_stable_promotion_observation_contract_never_prompts(self) -> None:
    ...
    def test_stable_promotion_runtime_admission_never_prompts(self) -> None:
        for mode in (PermissionMode.MODERATE, PermissionMode.BYPASS):
            decision = PermissionChecker(mode).check(
    ...
    def test_outcome_opportunity_permission_is_bounded_and_bypass_is_direct(
        self,
    ) -> None:
        arguments = {"outcome_id": "evrerollbackout_" + "a" * 24}
        moderate = PermissionChecker(PermissionMode.MODERATE).check(
    ...
    def test_goal_need_opportunity_permission_is_bounded_and_bypass_is_direct(
        self,
    ) -> None:
        arguments = {"goal_id": "goal_" + "a" * 12}
        moderate = PermissionChecker(PermissionMode.MODERATE).check(
