...
def _services(tmp_path: Path, outcome_service, clock):
    ...
    intent_service = window_service.exposure_service.deployment_service.intent_service
    plan_service = intent_service.store.plan_service
    ...
    service = EvolutionRevalidationStableStageCompletionService(
        workspace_root=tmp_path,
        outcome_service=outcome_service,
        window_service=window_service,
        plan_service=plan_service,
...
async def test_stable_completion_aggregates_runs_and_revalidates_sources(
    tmp_path: Path,
    ...
    plan = (
        await plan_service.inspect(plan_id=exposure.deployment.preparation.intent.plan.plan_id)
    ).plan
    required = plan.stages[3].minimum_completed_runs
    runs = []
    for index in range(required):
        ...
        assert outcome.stable_completed_run_authority

    views = await asyncio.gather(
        *(
            candidate.assess(intent_id=intent_id, subject_id=subject_id)
    ...
    assert receipt.stage_order == 4
    assert receipt.exposure_percent == 100
    assert receipt.population_snapshot_id == (
        exposure.deployment.preparation.intent.population_snapshot_id
    )
    assert receipt.installation_member_id == (
    ...
    assert receipt.metrics.insufficient_reasons == ("cost_baseline_not_comparable",)
    ...
    assert await store.latest(intent_id) == receipt
    ...
    breached = await service.assess(intent_id=intent_id, subject_id=subject_id)
    assert breached.receipt.metrics.status is EvolutionRevalidationStageCompletionStatus.BREACHED
    ...
    async with aiosqlite.connect(store.db_path) as db:
        ...
        await db.commit()
    with pytest.raises(EvolutionRevalidationStableStageCompletionError) as corrupt:
        await store.get(breached.receipt.evidence_id)
    assert corrupt.value.code == "stable_stage_completion_source_invalid"
