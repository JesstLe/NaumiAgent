...
from tests.unit.test_evolution_revalidation_percentage_observation_window_assessments import (
    _assessment_service,
)
from tests.unit.test_evolution_revalidation_percentage_observation_windows import (
    _started_exposure,
)


async def _sources(tmp_path: Path):
    subject_id = "percentage-execution-primary"
    ...
    chat_store = ChatRunStore(tmp_path / "percentage-chat-runs.db")
    outcome_store = EvolutionRevalidationPercentageExecutionOutcomeLedgerStore(
        exposure_store.db_path,
        window_store=window_store,
        chat_run_store=chat_store,
        harness_store=data["harness_store"],
    )
    service = EvolutionRevalidationPercentageExecutionOutcomeLedgerService(
    ...
    receipt = CompletionReceipt.from_dict(
        {
            ...
            "run_id": run.id,
            "outcome": outcome,
            "summary": "真实 percentage managed run 已结束。",
...
async def test_percentage_outcome_records_real_runs_and_revalidates_sources(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    data, lifecycle, exposure, chat_store, store, service = await _sources(tmp_path)
    assignment_id = data["assignment_id"]
    ...
    assert view.percentage_cohort_observation_input_authority
    assert view.successful_completed_run_authority
    assert view.percentage_completed_run_authority
    ...
    assert not view.receipt.percentage_stage_completion_authority
    assert not view.receipt.percentage_rollout_authority
    assert EvolutionRevalidationPercentageExecutionOutcome.model_validate_json(
        view.receipt.model_dump_json()
    ) == view.receipt
    assert await store.get_by_run(completed_run.id) == view.receipt
    assert await store.list_for_assignment(assignment_id) == (view.receipt,)
    with pytest.raises(ValueError, match="assignment_id"):
        await store.list_for_assignment("' OR 1=1 --")
    ...
    assert not failed.percentage_completed_run_authority
    ...
    assert pending.value.code == "percentage_execution_coverage_pending"
    ...
    async with aiosqlite.connect(store.db_path) as db:
        ...
        await db.commit()
    with pytest.raises(EvolutionRevalidationPercentageExecutionOutcomeError) as duplicate:
        await service.record(
            assignment_id=assignment_id,
            subject_id=subject_id,
    ...
    assert duplicate.value.code == "percentage_execution_cross_stage_duplicate"
    ...
    async with aiosqlite.connect(store.db_path) as db:
        await db.execute(
            "UPDATE evolution_revalidation_percentage_execution_outcomes "
        ...
        await db.commit()
    with pytest.raises(EvolutionRevalidationPercentageExecutionOutcomeError) as corrupt:
        await store.get(view.receipt.outcome_id)
    assert corrupt.value.code == "percentage_execution_outcome_source_invalid"
