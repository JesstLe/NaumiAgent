...
def test_new_ui_command_index_is_complete_deterministic_and_unique() -> None:
    ...
    assert by_name["/evolution"].arguments.syntax == (
        "[list|priorities|detail|discover-*|evaluation|stable-*|remote-*|rollout-*|decision-*|"
        "reflection-*|promotion-*|approval-request|approval-principal|approval-decision|"
        "approval-signature|approval-*|revalidation-rollback-execute|"
        "revalidation-rollback-outcome|revalidation-*|outcome-*|enqueue] [arguments]"
    )
