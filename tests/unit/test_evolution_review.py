...
def test_review_filter_rejects_unbounded_or_unknown_values() -> None:
    ...
    assert (
        EvolutionReviewFilter(source_kind="tool_catalog_miss").source_kind
        == "tool_catalog_miss"
    )
...
def test_agent_tools_keep_read_and_write_authority_separate(tmp_path: Path) -> None:
    ...
    assert [tool.name for tool in tools] == [
        ...
        "evolution_capability_artifact",
        "evolution_capability_scenario_binding",
    ...
    assert isinstance(tools[77], EvolutionToolCatalogMissOpportunityTool)
    assert isinstance(tools[78], EvolutionProposalQueueTool)
