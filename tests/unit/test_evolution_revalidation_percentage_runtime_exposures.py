...
from tests.unit.test_evolution_revalidation_percentage_deployments import (
    _deployment_context,
)
from tests.unit.test_release_launcher import _active_store


async def _exposure_context(tmp_path: Path):
    context, intent_view, _preparation_view, deployment_now, build_deployment = (
        await _deployment_context(tmp_path)
    )
    deployment_service, deployment_store = build_deployment()
    assignment_id = intent_view.intent.assignment.assignment_id
    deployment_view = await deployment_service.deploy(assignment_id=assignment_id)
    deployment_now[0] += timedelta(seconds=1)

    target = context["slots"].resolve_active_backend()
    runtime_now = [deployment_now[0]]
    ...
    harness_store = HarnessStore(tmp_path / "percentage-harness.db")
    ...
    def build_exposure_service():
        ...
        service = EvolutionRevalidationPercentageRuntimeExposureService(
            workspace_root=tmp_path,
...
async def test_managed_terminal_startup_records_percentage_exposure(
    tmp_path: Path,
) -> None:
    data = await _exposure_context(tmp_path)
    lifecycle = data["factory"].create(
        surface="new_ui",
        identity="percentage-runtime-primary",
    )
    assert await lifecycle.start()
    first, store = data["build_exposure_service"]()
    second, _second_store = data["build_exposure_service"]()

    views = await asyncio.gather(
        *(
            (first if index % 2 == 0 else second).record(
                assignment_id=data["assignment_id"],
                subject_id="percentage-runtime-primary",
            )
    ...
    assert view.percentage_observation_input_authority
    assert view.receipt.installation_exposure_observed
    ...
    assert await store.get_by_assignment(data["assignment_id"]) == view.receipt
    ...
    stopped = await first.inspect(assignment_id=data["assignment_id"])
    ...
    assert not stopped.percentage_observation_input_authority


@pytest.mark.asyncio
@pytest.mark.skipif(os.name == "nt", reason="真实 release fixture 使用 POSIX executable")
async def test_wrong_release_and_incomplete_startup_cannot_create_exposure(
    tmp_path: Path,
) -> None:
    data = await _exposure_context(tmp_path)
    external_store = _active_store(tmp_path / "other-release")
    ...
    external = external_factory.create(
        surface="tui",
        identity="percentage-runtime-wrong-release",
    )
    assert await external.start()
    service, store = data["build_exposure_service"]()

    with pytest.raises(EvolutionRevalidationPercentageRuntimeExposureError) as wrong:
        await service.record(
            assignment_id=data["assignment_id"],
            subject_id="percentage-runtime-wrong-release",
        )
    assert wrong.value.code == "percentage_runtime_exposure_release_mismatch"
    assert await store.get_by_assignment(data["assignment_id"]) is None

    binding = build_runtime_release_binding(
        workspace_root=tmp_path,
        surface="new_ui",
        subject_id="percentage-runtime-starting-only",
        instance_id="percentage-runtime-starting-only",
        epoch=1,
    ...
    with pytest.raises(EvolutionRevalidationPercentageRuntimeExposureError) as pending:
        await service.record(
            assignment_id=data["assignment_id"],
            subject_id="percentage-runtime-starting-only",
        )
    assert pending.value.code == "percentage_runtime_exposure_not_ready"
    assert await store.get_by_assignment(data["assignment_id"]) is None
...
@pytest.mark.asyncio
@pytest.mark.skipif(os.name == "nt", reason="真实 release fixture 使用 POSIX executable")
async def test_runtime_observation_tamper_revokes_exposure_fact(
    tmp_path: Path,
) -> None:
    data = await _exposure_context(tmp_path)
    lifecycle = data["factory"].create(
        surface="tui",
        identity="percentage-runtime-tamper",
    )
    assert await lifecycle.start()
    service, _store = data["build_exposure_service"]()
    recorded = await service.record(
        assignment_id=data["assignment_id"],
        subject_id="percentage-runtime-tamper",
    )
    ...
    with sqlite3.connect(data["harness_store"].db_path) as db:
        db.execute(
            ...
            (
                ...
                "0" * 64,
                "percentage-runtime-tamper",
            ),
        )

    revoked = await service.inspect(assignment_id=data["assignment_id"])
