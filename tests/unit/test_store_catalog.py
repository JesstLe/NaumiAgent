...
def test_default_catalog_covers_physical_stores_without_duplicate_paths(
    ...
    assert harness.supported_schema_version == 4
...
def test_catalog_distinguishes_current_unversioned_and_future_sqlite(
    ...
    _write_sqlite(harness.path, user_version=4)
    ...
    assert observations["harness.evidence"].observed_schema_version == 4

    future_path = tmp_path / "future-harness.db"
    _write_sqlite(future_path, user_version=5)
