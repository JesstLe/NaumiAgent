...
def test_default_catalog_covers_physical_stores_without_duplicate_paths(
    ...
    assert workers.supported_schema_version == WORKER_REGISTRY_SCHEMA_VERSION == 3
    ...
    decisions = next(
        item for item in definitions if item.store_id == "runtime.permission_decisions"
    )
    ...
    assert tool_jobs.supported_schema_version == TOOL_JOB_SCHEMA_VERSION == 3
    ...
    assert agent_jobs.supported_schema_version == AGENT_JOB_SCHEMA_VERSION == 1
